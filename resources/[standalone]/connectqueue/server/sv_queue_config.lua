...
function loadDatabaseQueue()
	N1Core.Functions.ExecuteSql(false, "SELECT * FROM `queue`", function(result)
...
N1Core.Commands.Add("reloadqueuepriority", "Give queue priority", {{name="id", help="ID of the player"}, {name="priority", help="Priority level"}}, true, function(source, args)
	loadDatabaseQueue()
	TriggerClientEvent('chatMessage', source, "SYSTEM", "normal", "REFRESH")	
end, "god")

N1Core.Commands.Add("addpriority", "Give queue priority", {{name="id", help="ID of the player"}, {name="priority", help="Priority level"}}, true, function(source, args)
...
N1Core.Commands.Add("removepriority", "Take priority away from someone", {{name="id", help="ID of the player"}}, true, function(source, args)
	local Player = N1Core.Functions.GetPlayer(tonumber(args[1]))
	if Player ~= nil then
...
function AddPriority(source, level)
	...
	if Player ~= nil then 
		...
		N1Core.Functions.ExecuteSql(true, "DELETE FROM `queue` WHERE `steam` = '"..GetPlayerIdentifiers(source)[1].."'")
		N1Core.Functions.ExecuteSql(true, "INSERT INTO `queue` (`name`, `steam`, `license`, `priority`) VALUES ('"..GetPlayerName(source).."', '"..GetPlayerIdentifiers(source)[1].."', '"..GetPlayerIdentifiers(source)[2].."', '"..level.."')")
