...
AddEventHandler('prison:server:SetJailStatus', function(jailTime)
    ...
    if jailTime > 0 then
        if Player.PlayerData.job.name ~= "unemployed" then
            ...
            TriggerClientEvent('N1Core:Notify', src, "You are unemployed..")
        end
    end
end)
...
N1Core.Functions.CreateUseableItem("electronickit", function(source, item)
...
N1Core.Functions.CreateCallback('prison:server:IsAlarmActive', function(source, cb)
    cb(AlarmActivated)
end)