...
RegisterNetEvent('N1-fitbit:use')
AddEventHandler('N1-fitbit:use', function()
  openWatch(true)
end)
...
RegisterNUICallback('setFoodWarning', function(data)
    ...
    TriggerServerEvent('N1-fitbit:server:setValue', 'food', foodValue)

    N1Core.Functions.Notify('Fitbit: Food warning set to '..foodValue..'%')
end)

RegisterNUICallback('setThirstWarning', function(data)
    local thirstValue = tonumber(data.value)

    TriggerServerEvent('N1-fitbit:server:setValue', 'thirst', thirstValue)

    N1Core.Functions.Notify('Fitbit: Thirst warning set to'..thirstValue..'%')
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            N1Core.Functions.TriggerCallback('N1-fitbit:server:HasFitbit', function(hasItem)
                if hasItem then
