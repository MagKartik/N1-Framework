...
N1Fitbit.Open = function() {
    $(".container").fadeIn(150);
}

N1Fitbit.Close = function() {
    $(".container").fadeOut(150);
    $.post('http://N1-fitbit/close')
}
...
$(document).on('click', '.save-food-settings', function(e){
    ...
    if (parseInt(foodValue.val()) <= 100) {
        $.post('http://N1-fitbit/setFoodWarning', JSON.stringify({
            value: foodValue.val()
        }));
    }
});
...
$(document).on('click', '.save-thirst-settings', function(e){
    ...
    if (parseInt(thirstValue.val()) <= 100) {
        $.post('http://N1-fitbit/setThirstWarning', JSON.stringify({
