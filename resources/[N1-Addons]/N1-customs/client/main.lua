...
Citizen.CreateThread(function()
    for k, v in pairs(N1Customs.Locations) do
...
function OpenCustoms(shop, k)
    ...
    CurrentVehicleData = N1Core.Functions.GetVehicleProperties(veh)
    ...
    if k == 7 then
        N1Core.Functions.TriggerCallback('N1-vehicletuning:server:IsMechanicAvailable', function(Mechanic)
            ...
            TriggerServerEvent('N1-customs:server:UpdateBusyState', k, true)
            CurrentShop = k
        end)
    else
        SetNuiFocus(true, false)
...
RegisterNUICallback('CanRepairVehicle', function(data, cb)
    ...
    N1Core.Functions.TriggerCallback('N1-customs:server:CanPurchase', function(CanBuy)
        if CanBuy then
	        SetVehicleFixed(veh)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "airwrench", 0.1)
            cb(true)
        else
            N1Core.Functions.Notify('You dont have enough money..', 'error')
            cb(false)
        end
    end, data.price)
end)
...
function IsVehicleBlacklisted(veh)
    local retval = false
    for _, vehicle in pairs(N1Customs.BlacklistedVehicles) do
...
function GetAvailableMods()
    ...
    if N1Core.Shared.VehicleModels[GetEntityModel(veh)] ~= nil then
        model = N1Core.Shared.VehicleModels[GetEntityModel(veh)].model
        VehicleData = N1Core.Shared.Vehicles[model]
    ...
    local mods = {}
    for k, v in pairs(N1Customs.Mods) do
                            ...
                            table.insert(mods, {
                                ...
                                buttons = {
                                    ...
                                    },
                                    {
                                        id = "wheelcolors",
                                        name = "Wheel Colors",
                                        buttons = N1Customs.WheelColors,
                                    },
                                    {
                                        id = "wheelaccessories",
                                        name = "Wheel Accessoires",
                                        buttons = N1Customs.WheelAccessories,
                                    },
                                },
                            })
                        else
...
Citizen.CreateThread(function()
    while true do
        ...
        for k, loc in pairs(N1Customs.Locations) do
...
RegisterNUICallback('CloseMenu', function()
    ...
    TriggerServerEvent('N1-customs:server:UpdateBusyState', CurrentShop, false)
    ShoppingCart.buttons = {}
    N1Core.Functions.SetVehicleProperties(veh, CurrentVehicleData)
    CurrentShop = nil
end)

RegisterNUICallback('print', function(data, cb)
    -- TriggerServerEvent('N1-customs:print', data.print)
end)
...
RegisterNetEvent('N1-customs:client:UpdateBusyState')
AddEventHandler('N1-customs:client:UpdateBusyState', function(k, bool)
    ...
    if data.data ~= nil and next(data.data) ~= nil then
        if data.data.id ~= nil then
            if data.data.id ~= "respray" and data.data.id ~= "wheels" and data.data.id ~= "wheelcolors" and data.data.id ~= "wheelaccessories" then
                if data.data.id >= 0 and data.data.id <= 46 then
                    if data.data.id == 18 or data.data.id == 20 or data.data.id == 22 then
                        if not CurrentVehicleData[N1Customs.indexToName[data.data.id]] then
                            currentmod = -1
                        else
                            currentmod = 0
                        end
                    else
                        currentmod = CurrentVehicleData[N1Customs.indexToName[data.data.id]]
                    end
                end
            ...
            elseif data.data.id == "wheelcolors" then
                local pearlescentColor, wheelColor = GetVehicleExtraColours(veh)
                for k, v in pairs(N1Customs.WheelColors) do
    ...
    N1Core.Functions.TriggerCallback('N1-customs:server:CanPurchase', function(CanBuy)
        if CanBuy then
            ...
            if ShoppingCart.buttons ~= nil and next(ShoppingCart.buttons) ~= nil then
                ...
                TriggerServerEvent("N1-log:server:sendLog", PlayerData.citizenid, "buy", {upgrades = String, citizenid = PlayerData.citizenid})
	            TriggerServerEvent("N1-log:server:CreateLog", "bennys", "buy", "green", "**" .. PlayerData.name .. "** (citizenid: *" .. PlayerData.citizenid .. "* | id: *(" .. PlayerData.source .. ")* has paid **₹" .. TotalPrice .. "** for the following upgrade\'s: \n" .. String)
            end
            ...
            TriggerServerEvent('N1-customs:server:SaveVehicleProps', CurrentVehicleData)
        else
            N1Core.Functions.Notify('You dont have enough money..', 'error')
        end
    end, TotalPrice)
...
function GetTyreSmokeKey()
    ...
    for k, v in pairs(N1Customs.WheelAccessories) do
    ...
    if data.data ~= nil then
        if ShoppingCart.buttons ~= nil and next(ShoppingCart.buttons) ~= nil then
            for k, v in pairs(ShoppingCart.buttons) do
                ...
                elseif v.modtype == "wheelcolor" then
                    for k, mod in pairs(data.data) do
                        for _, color in pairs(N1Customs.WheelColors) do
