...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('http://N1-Phone /ClearRecentAlerts');
...
N1.Phone.Functions.SetupRecentCalls = function(recentcalls) {
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    console.log(N1.Phone.Data.AnonymousCall)
            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        N1.Phone.Notifications.Add("fas fa-phone", "Phone", "You're already in a call!");
                    }
                } else {
                    N1.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is busy!");
                }
            } else {
                N1.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
        } else {
            N1.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call yourself!");
        }
    });
});
...
N1.Phone.Functions.LoadContacts = function(myContacts) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== N1.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            N1.Phone.Functions.ToggleApp("phone", "none");
            N1.Phone.Functions.ToggleApp("whatsapp", "block");
            N1.Phone.Data.currentApplication = "whatsapp";
        
            $.post('http://N1-Phone /GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
                N1.Phone.Functions.SetupChatMessages(chat, {
        ...
        N1.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "You can't whatsapp yourself..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    N1.Phone.Animations.TopSlideDown(".phone-edit-contact", 200, 0);
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://N1-Phone /EditContact', JSON.stringify({
            ...
            N1.Phone.Functions.LoadContacts(PhoneContacts);
        });
        N1.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
        setTimeout(function(){
            $(".phone-edit-contact-number").val("");
            $(".phone-edit-contact-name").val("");
        }, 250)
    } else {
        N1.Phone.Notifications.Add("fas fa-exclamation-circle", "Edit Contact", "Fill out all fields!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://N1-Phone /DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#phone-plus-icon', function(e){
    ...
    N1.Phone.Animations.TopSlideDown(".phone-add-contact", 200, 0);
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://N1-Phone /AddNewContact', JSON.stringify({
        ...
        N1.Phone.Animations.TopSlideUp(".phone-add-contact", 250, -100);
        setTimeout(function(){
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://N1-Phone /RemoveSuggestion', JSON.stringify({
        ...
        N1.Phone.Notifications.Add("fas fa-exclamation-circle", "Add Contact", "Fill out all fields!");
    }
});
        ...
        if (cData.number !== N1.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    N1.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is in a call!");
                }
            } else {
                N1.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
...
CancelOutgoingCall = function() {
    if (N1.Phone.Data.currentApplication == "phone-call") {
        N1.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
    ...
    if (!Canceled) {
        if (!N1.Phone.Data.CallActive) {
            ...
            setTimeout(function(){
                ...
                $(".phone-app").css({"display":"none"});
                N1.Phone.Functions.HeaderTextColor("white", 400);
                $("."+N1.Phone.Data.currentApplication+"-app").css({"display":"none"});
...
N1.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        CallData = cData;
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    N1.Phone.Functions.HeaderTextColor("white", 500);
    N1.Phone.Animations.TopSlideDown('.phone-application-container', 500, 0);
    N1.Phone.Animations.TopSlideDown('.phone-call-app', 500, 0);
    N1.Phone.Functions.ToggleApp("phone-call", "block");
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('http://N1-Phone /AnswerCall');
});

N1.Phone.Functions.AnswerCall = function(CallData) {
    $(".phone-call-incoming").css({"display":"none"});
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
...
N1.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
