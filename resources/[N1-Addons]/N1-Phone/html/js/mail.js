...
$(document).on('click', '.mail', function(e){
    ...
    N1.Phone.Functions.SetupMail(MailData);
...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('http://N1-Phone /AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('http://N1-Phone /RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
N1.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(N1.Phone.Data.PlayerData.charinfo.firstname+"."+N1.Phone.Data.PlayerData.charinfo.lastname+"@N1us.nl");
...
N1.Phone.Functions.SetupMail = function(MailData) {
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('http://N1-Phone /PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
        N1.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You can\'t post an empty ad!", "#ff8f1a", 2000);
    }
});

N1.Phone.Functions.RefreshAdverts = function(Adverts) {
    $("#advert-header-name").html("@"+N1.Phone.Data.PlayerData.charinfo.firstname+""+N1.Phone.Data.PlayerData.charinfo.lastname+" | "+N1.Phone.Data.PlayerData.charinfo.phone);
