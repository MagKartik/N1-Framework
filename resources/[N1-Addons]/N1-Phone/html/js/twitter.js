...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('http://N1-Phone /ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    N1.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});

N1.Phone.Notifications.LoadTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        $(".twitter-home-tab").html("");
        $.each(Tweets, function(i, Tweet){
            var TwtMessage = N1.Phone.Functions.FormatTwitterMessage(Tweet.message);
            var today = new Date();
...
N1.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets.length > 0) {
...
N1.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('http://N1-Phone /PostNewTweet', JSON.stringify({
            Message: TweetMessage,
            Date: CurrentDate,
            Picture: N1.Phone.Data.MetaData.profilepicture
        }), function(Tweets){
            N1.Phone.Notifications.LoadTweets(Tweets);
        });
        ...
        N1.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
        N1.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Fill a message!", "#1DA1F2");
    }
});
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://N1-Phone /GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            N1.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    N1.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag')+ " copied!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

N1.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
        ...
        $.each(hashtags, function(i, hashtag){
            ...
            if (hashtag.messages.length >= MinimumTrending) {
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Trending in N1us</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            } else {
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Not trending in N1us</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            }
...
N1.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    N1.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
