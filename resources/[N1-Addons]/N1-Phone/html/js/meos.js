...
SetupMeosHome = function() {
    $("#meos-app-name").html("Welcome " + N1.Phone.Data.PlayerData.charinfo.firstname + " " + N1.Phone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.house-adress-location', function(e){
    ...
    $.post('http://N1-Phone /SetGPSLocation', JSON.stringify({
...
$(document).on('click', '.appartment-adress-location', function(e){
    ...
    $.post('http://N1-Phone /SetApartmentLocation', JSON.stringify({
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    N1.Phone.Notifications.Add("fas fa-university", "MDT", "House number copied!", "#badc58", 1750);
...
$(document).on('click', '.confirm-search-person-test', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://N1-Phone /FetchSearchResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
                ...
                N1.Phone.Notifications.Add("politie", "MDT", "There are no search results!");
                $(".person-search-results").html("");
            }
        });
    } else {
...
$(document).on('click', '.confirm-search-person-house', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://N1-Phone /FetchPlayerHouses', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
...
$(document).on('click', '.confirm-search-vehicle', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://N1-Phone /FetchVehicleResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".vehicle-search-results").html("");
...
$(document).on('click', '.scan-search-vehicle', function(e){
    e.preventDefault();
    $.post('http://N1-Phone /FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            $(".vehicle-search-results").html("");
            var APK = "Yes";
            if (!vehicle.status) {
            ...
            N1.Phone.Notifications.Add("politie", "MDT", "No vehicle nearby!");
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData.coords != undefined && alertData.coords != null) {
        $.post('http://N1-Phone /SetAlertWaypoint', JSON.stringify({
            alert: alertData,
        }));
    } else {
        N1.Phone.Notifications.Add("politie", "MDT", "This alert doesn't have a GPS location!");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    ...
    N1.Phone.Notifications.Add("politie", "MDT", "All alerts have been deleted!");
});