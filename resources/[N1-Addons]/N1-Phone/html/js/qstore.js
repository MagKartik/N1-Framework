...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    N1.Phone.Data.Applications[AppData.app] = null;

    $.post('http://N1-Phone /RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
...
$(document).on('click', '.download-password-accept', function(e){
    ...
    if (FilledInPassword == TemplatePassword) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('http://N1-Phone /InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                if (Installed) {
                    var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, N1.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === N1.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    N1.Phone.Data.Applications[Installed.data.app] = Installed.data;
