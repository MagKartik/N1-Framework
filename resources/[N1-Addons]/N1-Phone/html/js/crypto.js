...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Wallet: "+CryptoData.Portfolio+" N1it('s)");
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (N1.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('http://N1-Phone /BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    N1.Phone.Data.PlayerData.money.bank = parseInt(N1.Phone.Data.PlayerData.money.bank) - parseInt(Price);
                    N1.Phone.Notifications.Add("fas fa-university", "N1ank", "&euro; "+Price+",- has been withdrawn from your balance!", "#badc58", 2500);
                } else {
                    N1.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough money..", "#badc58", 1500);
                }
            });
        } else {
            N1.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough money..", "#badc58", 1500);
        ...
        N1.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill out all fields!", "#badc58", 1500);
    }
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('http://N1-Phone /SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    N1.Phone.Data.PlayerData.money.bank = parseInt(N1.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    N1.Phone.Notifications.Add("fas fa-university", "N1ank", "&euro; "+Price+",- has been added to your balance!", "#badc58", 2500);
                } else {
                    N1.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough N1its..", "#badc58", 1500);
                }
            });
        } else {
            N1.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough N1its..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('http://N1-Phone /TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        N1.Phone.Notifications.Add("fas fa-university", "Crypto", "this Wallet-ID doesn't exist!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        N1.Phone.Notifications.Add("fas fa-university", "Crypto", "You transferred "+Coins+",- to "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                N1.Phone.Notifications.Add("fas fa-university", "Crypto", "You can't transfer to yourself..", "#badc58", 2500);
            }
        } else {
            N1.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough N1its..", "#badc58", 1500);
        }
