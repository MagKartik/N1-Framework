...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    N1.Phone.Functions.SetupChatMessages(ChatData);

    $.post('http://N1-Phone /ClearAlerts', JSON.stringify({
...
N1.Phone.Functions.GetLastMessage = function(messages) {
...
N1.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = N1.Phone.Functions.GetLastMessage(chat.messages);
...
N1.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('http://N1-Phone /SendMessage', JSON.stringify({
        ...
        N1.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "You can't send a empty message!", "#25D366", 1750);
    }
});
...
// N1.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
//     if (cData) {
//         OpenedChatData.number = cData.number;

//         if (OpenedChatPicture == null) {
//             $.post('http://N1-Phone /GetProfilePicture', JSON.stringify({
...
//                 var Sender = "me";
//                 if (message.sender !== N1.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
//                 var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('http://N1-Phone /SharedLocation', JSON.stringify({
