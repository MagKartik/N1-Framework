...
$(document).on('click', '.bank-app-account', function(e){
    ...
    N1.Phone.Notifications.Add("fas fa-university", "N1ank", "Account number. copied!", "#badc58", 1750);
});
...
N1.Phone.Functions.DoBankOpen = function() {
    N1.Phone.Data.PlayerData.money.bank = (N1.Phone.Data.PlayerData.money.bank).toFixed();
    $(".bank-app-account-number").val(N1.Phone.Data.PlayerData.charinfo.account);
    $(".bank-app-account-balance").html("&euro; "+N1.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance").data('balance', N1.Phone.Data.PlayerData.money.bank);

    $(".bank-app-loaded").css({"display":"none", "padding-left":"30vh"});
    $(".bank-app-accounts").css({"left":"30vh"});
    $(".N1ank-logo").css({"left": "0vh"});
    $("#N1ank-text").css({"opacity":"0.0", "left":"9vh"});
...
$(document).on('click', '.bank-app-account-actions', function(e){
    N1.Phone.Animations.TopSlideDown(".bank-app-transfer", 400, 0);
});

$(document).on('click', '#cancel-transfer', function(e){
    e.preventDefault();

    N1.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
            $.post('http://N1-Phone /CanTransferMoney', JSON.stringify({
                sendTo: iban,
                amountOf: amount,
            }), function(data){
                if (data.TransferedMoney) {
                    ...
                    $(".bank-app-account-balance").html("&euro; " + (data.NewBalance).toFixed(0));
                    $(".bank-app-account-balance").data('balance', (data.NewBalance).toFixed(0));
                    N1.Phone.Notifications.Add("fas fa-university", "N1ank", "You have transfered &euro; "+amount+",-!", "#badc58", 1500);
                } else {
                    N1.Phone.Notifications.Add("fas fa-university", "N1ank", "You don't have enough balance!", "#badc58", 1500);
                }
                N1.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        N1.Phone.Notifications.Add("fas fa-university", "N1ank", "Fill out all fields!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://N1-Phone /PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                N1.Phone.Notifications.Add("fas fa-university", "N1ank", "You have paid &euro;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('http://N1-Phone /DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        invoiceId: InvoiceData.invoiceid,
    }));
...
N1.Phone.Functions.LoadContactsWithNumber = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            var ContactColor = N1.Phone.ContactColors[RandomNumber];
...
$(document).on('click', '.bank-app-my-contacts-list-back', function(e){
    ...
    N1.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});

$(document).on('click', '.bank-transfer-mycontacts-icon', function(e){
    e.preventDefault();

    N1.Phone.Animations.TopSlideDown(".bank-app-my-contacts", 400, 0);
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        N1.Phone.Notifications.Add("fas fa-university", "N1ank", "There is no bank account attached to this number!", "#badc58", 2500);
    }
    N1.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});