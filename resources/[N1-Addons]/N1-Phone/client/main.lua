...
RegisterNetEvent('N1-phone_new:client:RaceNotify')
AddEventHandler('N1-phone_new:client:RaceNotify', function(message)
...
RegisterNetEvent('N1-phone_new:client:AddRecentCall')
AddEventHandler('N1-phone_new:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('N1-phone:server:SetPhoneAlerts', "phone")
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('N1-phone:server:SetPhoneAlerts', "phone", 0)
...
RegisterNUICallback('SetBackground', function(data)
    ...
    TriggerServerEvent('N1-phone_new:server:SaveMetaData', PhoneData.MetaData)
end)
        ...
        if IsControlJustPressed(0, Config.OpenPhone) then
            if not PhoneData.isOpen then
                ...
                if not IsHandcuffed then
                    ...
                    N1Core.Functions.Notify("Action not available at the moment..", "error")
                end
            end
        end
        Citizen.Wait(3)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            N1Core.Functions.TriggerCallback('N1-phone_new:server:GetPhoneData', function(pData)   
...
function LoadPhone()
    ...
    isLoggedIn = true
    N1Core.Functions.TriggerCallback('N1-phone_new:server:GetPhoneData', function(pData)
        PlayerJob = N1Core.Functions.GetPlayerData().job
        PhoneData.PlayerData = N1Core.Functions.GetPlayerData()
...
RegisterNUICallback('HasPhone', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-phone_new:server:HasPhone', function(HasPhone)
        cb(HasPhone)
    end)
end)
...
function OpenPhone()
    N1Core.Functions.TriggerCallback('N1-phone_new:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            N1Core.Functions.TriggerCallback('N1-phone_new:server:GetGarageVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            N1Core.Functions.Notify("You don't have a phone", "error")
        end
    end)
end
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('N1-phone_new:server:RemoveMail', MailId)
    cb('ok')
end)

RegisterNetEvent('N1-phone_new:client:UpdateMails')
AddEventHandler('N1-phone_new:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNUICallback('AcceptMailButton', function(data)
    ...
    TriggerServerEvent('N1-phone_new:server:ClearButtonData', data.mailId)
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('N1-phone_new:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    N1Core.Functions.TriggerCallback('N1-phone_new:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('N1-phone_new:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('N1-phone_new:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    N1Core.Functions.TriggerCallback('N1-phone_new:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNetEvent('N1-phone_new:client:UpdateMessages')
AddEventHandler('N1-phone_new:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            Wait(100)
            N1Core.Functions.TriggerCallback('N1-phone_new:server:GetContactPictures', function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
            ...
            TriggerServerEvent('N1-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
...
RegisterNetEvent("N1-phone-new:client:BankNotify")
AddEventHandler("N1-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
        action = "Notification",
        NotifyData = {
            title = "Bank", 
...
RegisterNetEvent('N1-phone_new:client:NewMailNotify')
AddEventHandler('N1-phone_new:client:NewMailNotify', function(MailData)
    if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
    ...
    TriggerServerEvent('N1-phone:server:SetPhoneAlerts', "mail")
end)

RegisterNUICallback('PostAdvert', function(data)
    TriggerServerEvent('N1-phone_new:server:AddAdvert', data.message)
end)

RegisterNetEvent('N1-phone_new:client:UpdateAdverts')
AddEventHandler('N1-phone_new:client:UpdateAdverts', function(Adverts, LastAd)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('N1-phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    N1Core.Functions.TriggerCallback('N1-phone_new:server:PayInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    N1Core.Functions.TriggerCallback('N1-phone_new:server:DeclineInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Citizen.Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('N1-phone_new:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
...
RegisterNetEvent('N1-phone_new:client:UpdateHashtags')
AddEventHandler('N1-phone_new:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNUICallback('PostNewTweet', function(data, cb)
    ...
    for i = 2, #Hashtag, 1 do
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            TriggerServerEvent('N1-phone_new:server:UpdateHashtags', Handle, TweetMessage)
        end
    end
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    TriggerServerEvent('N1-phone_new:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                else
    ...
    TriggerServerEvent('N1-phone_new:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
end)

RegisterNetEvent('N1-phone_new:client:TransferMoney')
AddEventHandler('N1-phone_new:client:TransferMoney', function(amount, newmoney)
    PhoneData.PlayerData.money.bank = newmoney
    if PhoneData.isOpen then
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "N1ank", text = "&euro;"..amount.." has been added to your account!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = PhoneData.PlayerData.money.bank })
    else
        SendNUIMessage({ action = "Notification", NotifyData = { title = "N1ank", content = "&euro;"..amount.." has been added to your account!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)


RegisterNetEvent('N1-phone_new:client:UpdateTweets')
AddEventHandler('N1-phone_new:client:UpdateTweets', function(src, Tweets, NewTweetData)
...
RegisterNetEvent('N1-phone_new:client:GetMentioned')
AddEventHandler('N1-phone_new:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNUICallback('ClearMentions', function()
    ...
    TriggerServerEvent('N1-phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('N1-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('N1-phone_new:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        N1Core.Functions.TriggerCallback('N1-phone_new:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('CallContact', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-phone_new:server:GetCallState', function(CanCall, IsOnline)
        local status = { 
            CanCall = CanCall, 
            IsOnline = IsOnline,
...
CallContact = function(CallData, AnonymousCall)
    ...
    TriggerServerEvent('N1-phone_new:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('N1-phone_new:server:SetCallState', true)
...
CancelCall = function()
    TriggerServerEvent('N1-phone_new:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('N1-phone_new:server:SetCallState', false)
...
RegisterNetEvent('N1-phone_new:client:CancelCall')
AddEventHandler('N1-phone_new:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNetEvent('N1-phone_new:client:GetCalled')
AddEventHandler('N1-phone_new:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('N1-phone_new:server:AddRecentCall', "missed", CallData)
                    break
                end
                Citizen.Wait(Config.RepeatTimeout)
            else
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('N1-phone_new:server:AnswerCall', PhoneData.CallData)
...
RegisterNetEvent('N1-phone_new:client:AnswerCall')
AddEventHandler('N1-phone_new:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-phone_new:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)

RegisterNUICallback('FetchVehicleResults', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-phone_new:server:GetVehicleSearchResults', function(result)
        if result ~= nil then 
            for k, v in pairs(result) do
                N1Core.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    N1Core.Functions.TriggerCallback('N1-phone_new:server:ScanPlate', function(result)
        N1Core.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
            local vehicleInfo = N1Core.Shared.Vehicles[N1Core.Shared.VehicleModels[model]["model"]] ~= nil and N1Core.Shared.Vehicles[N1Core.Shared.VehicleModels[model]["model"]] or {["brand"] = "Unknown brand..", ["name"] = ""}
...
RegisterNetEvent('N1-phone:client:addPoliceAlert')
AddEventHandler('N1-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = N1Core.Functions.GetPlayerData().job
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    N1Core.Functions.Notify('GPS Locatie ingesteld: '..data.alert.title)
...
RegisterNetEvent('N1-phone_new:client:GiveContactDetails')
AddEventHandler('N1-phone_new:client:GiveContactDetails', function()
    local ped = GetPlayerPed(-1)

    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('N1-phone_new:server:GiveContactDetails', PlayerId)
    else
        N1Core.Functions.Notify("No one nearby!", "error")
    end
end)

-- Citizen.CreateThread(function()
--     Wait(1000)
--     TriggerServerEvent('N1-phone_new:server:GiveContactDetails', 1)
-- end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('N1-phone_new:server:RemoveContact', Name, Number)
end)

RegisterNetEvent('N1-phone_new:client:AddNewSuggestion')
AddEventHandler('N1-phone_new:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('N1-phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNetEvent('N1-phone_new:client:RemoveBankMoney')
AddEventHandler('N1-phone_new:client:RemoveBankMoney', function(amount)
    if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('N1-phone_new:client:AddTransaction')
AddEventHandler('N1-phone_new:client:AddTransaction', function(SenderData, TransactionData, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('N1-phone_new:server:AddTransaction', Data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data)
    TriggerServerEvent('N1-lapraces:server:JoinRace', data.RaceData)
end)

RegisterNUICallback('LeaveRace', function(data)
    TriggerServerEvent('N1-lapraces:server:LeaveRace', data.RaceData)
end)

RegisterNUICallback('StartRace', function(data)
    TriggerServerEvent('N1-lapraces:server:StartRace', data.RaceData.RaceId)
end)

RegisterNetEvent('N1-phone_new:client:UpdateLapraces')
AddEventHandler('N1-phone_new:client:UpdateLapraces', function()
    SendNUIMessage({
...
RegisterNUICallback('GetRaces', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('N1-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
end)

RegisterNUICallback('HasCreatedRace', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(data, cb)
    local InRace = exports['N1-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        local data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['N1-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('N1-lapraces:server:CreateLapRace', data.TrackName)
end)

RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-lapraces:server:GetRacingData', function(RaceData)
        local ped = GetPlayerPed(-1)
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('N1-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            N1Core.Functions.Notify('You\'re too far away from the race. GPS has been set to the race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['N1-lapraces']:IsInEditor())
    else
        cb(exports['N1-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-phone_new:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-phone_new:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    N1Core.Functions.Notify("GPS has been set on " .. data.HouseData.HouseData.adress .. "!", "success")
end)

RegisterNUICallback('RemoveKeyholder', function(data)
    TriggerServerEvent('N1-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    N1Core.Functions.TriggerCallback('N1-phone_new:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-phone_new:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    N1Core.Functions.Notify('GPS has been set!', 'success')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    N1Core.Functions.TriggerCallback('N1-phone_new:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('N1-phone_new:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('N1-phone_new:server:RemoveInstallation', data.app)
end)

RegisterNetEvent('N1-phone_new:RefreshPhone')
AddEventHandler('N1-phone_new:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNUICallback('GetTruckerData', function(data, cb)
    local TruckerMeta = N1Core.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
    local TierData = exports['N1-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
