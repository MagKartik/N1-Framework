                    ...
                    if Races[k] ~= nil then
                        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[k].startx, Races[k].starty, Races[k].startz, true) < 15.0 and not Races[k].started then
                            ...
                            if IsControlJustReleased(0, Keys["H"]) then
                                TriggerServerEvent("N1-streetraces:JoinRace", k)
                            end
                        end
                    end
            ...
            if RaceId ~= 0 and InRace then
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 250.0 and Races[RaceId].started then
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 15.0 then
                        TriggerServerEvent("N1-streetraces:RaceWon", RaceId)
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('N1-streetraces:StartRace')
AddEventHandler('N1-streetraces:StartRace', function(race)
...
RegisterNetEvent('N1-streetraces:RaceDone')
AddEventHandler('N1-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        N1Core.Functions.Notify(" The race endded. "..winner.. "!")
    end
end)

RegisterNetEvent('N1-streetraces:StopRace')
AddEventHandler('N1-streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)

RegisterNetEvent('N1-streetraces:CreateRace')
AddEventHandler('N1-streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(GetPlayerPed(-1), true)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            TriggerServerEvent("N1-streetraces:NewRace", race)
            N1Core.Functions.Notify("Race gemaakt voor ₹"..amount..",-!", "success")
        else
            N1Core.Functions.Notify("end position to close!", "error")
        end
    else
        N1Core.Functions.Notify("You have to place a marker!", "error")
    end
end)

RegisterNetEvent('N1-streetraces:SetRace')
AddEventHandler('N1-streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('N1-streetraces:SetRaceId')
AddEventHandler('N1-streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        N1Core.Functions.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    N1Core.Functions.Notify("GOOOOOOOOO!!!")
end
