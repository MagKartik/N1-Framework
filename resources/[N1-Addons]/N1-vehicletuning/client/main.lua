    ...
    N1Core.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerData.job.name == "mechanic" then
                TriggerServerEvent("N1Core:ToggleDuty")
            end
        end
    end)
    isLoggedIn = true
    N1Core.Functions.TriggerCallback('N1-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    N1Core.Functions.TriggerCallback('N1-vehicletuning:server:GetDrivingDistances', function(retval)
...
RegisterNetEvent('N1Core:Client:SetDuty')
AddEventHandler('N1Core:Client:SetDuty', function(duty)
    onDuty = duty
end)
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            if IsControlJustPressed(0, Config.Keys["E"]) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                TriggerServerEvent('N1-vehicletuning:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            N1Core.Functions.Notify("You cant put a bike on the plate!", "error")
                                        end
                                    end
                                end
                            end
...
function SpawnListVehicle(model)
    ...
    N1Core.Functions.SpawnVehicle(model, function(veh)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repair ("..N1Core.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close menu", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    N1Core.Functions.TriggerCallback('N1-inventory:server:GetStashItems', function(StashItems)
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    N1Core.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." Repairing", math.random(5000, 10000), false, true, {
                        ...
                        TriggerEvent('N1-vehicletuning:client:RepaireeePart', part)
                        TriggerServerEvent('N1-inventory:server:SaveStashItems', "mechanicstash", StashItems)
                        SetTimeout(250, function()
                            PartsMenu()
                        end)
                    end, function()
                        N1Core.Functions.Notify("Repair canceled..", "error")
                    end)
                    break
                else
                    N1Core.Functions.Notify('There are not enough materials in the safe..', 'error')
                end
                break
            else
                N1Core.Functions.Notify('There are not enough materials in the safe..', 'error')
...
RegisterNetEvent('N1-vehicletuning:client:RepaireeePart')
AddEventHandler('N1-vehicletuning:client:RepaireeePart', function(part)
    ...
    N1Core.Functions.Notify("De "..Config.ValuesLabels[part].." is gerepareerd!")
end)
...
function UnattachVehicle()
    ...
    TriggerServerEvent('N1-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
end

RegisterNetEvent('N1-vehicletuning:client:SetAttachedVehicle')
AddEventHandler('N1-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        N1Core.Functions.Notify("No status know..", "error")
                    end
                else
                    N1Core.Functions.Notify("Not a valid vehicle..", "error")
                end
            else
                N1Core.Functions.Notify("You are not close enough to the vehicle..", "error")
            end
        else
            N1Core.Functions.Notify("You must be in the vehicle first..", "error")
        end
    else
        N1Core.Functions.Notify("You must be outside the vehicle..", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
            ...
            N1Core.Functions.Notify("You are not a driver or on a bicycle..", "error")
        end
    else
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            N1Core.Functions.Progressbar("repair_advanced", "Repair vehicle..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                if part == "body" then
                                    ...
                                    TriggerServerEvent("N1Core:Server:RemoveItem", Config.RepairCost[part], needAmount)
                                    TriggerEvent("inventory:client:ItemBox", N1Core.Shared.Items[Config.RepairCost[part]], "remove")
                                elseif part ~= "engine" then
                                    TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                    TriggerServerEvent("N1Core:Server:RemoveItem", Config.RepairCost[part], level)
                                    TriggerEvent("inventory:client:ItemBox", N1Core.Shared.Items[Config.RepairCost[part]], "remove")
                                end
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(GetPlayerPed(-1))
                                N1Core.Functions.Notify("Proces canceled..", "error")
                            end)
                        else
                            N1Core.Functions.Notify("Not a valid part..", "error")
                        end
                    else
                        N1Core.Functions.Notify("Not a valid vehicle..", "error")
                    end
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['N1-companies']:IsEmployee(v)
    end
    return retval
end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Voertuig Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..N1Core.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..N1Core.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..N1Core.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..N1Core.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..N1Core.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..N1Core.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..N1Core.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
