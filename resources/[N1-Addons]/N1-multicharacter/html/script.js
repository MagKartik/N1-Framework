    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            if (data.toggle) {
                ...
                setTimeout(function(){
                    $.post('http://N1-multicharacter/setupCharacters');
                    setTimeout(function(){
                        clearInterval(DotsInterval);
                        loadingProgress = 0;
                        originalText = "Retrieving data";
                        $(".welcomescreen").fadeOut(150);
                        N1MultiCharacters.fadeInDown('.character-info', '20%', 400);
                        N1MultiCharacters.fadeInDown('.characters-list', '20%', 400);
                        $.post('http://N1-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.continue-btn').click(function(e){
    ...
    // N1MultiCharacters.fadeOutUp('.welcomescreen', undefined, 400);
    // N1MultiCharacters.fadeOutDown('.server-log', undefined, 400);
    // setTimeout(function(){
    //     N1MultiCharacters.fadeInDown('.characters-list', '20%', 400);
    //     N1MultiCharacters.fadeInDown('.character-info', '20%', 400);
    //     $.post('http://N1-multicharacter/setupCharacters');
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('http://N1-multicharacter/closeUI');
    $.post('http://N1-multicharacter/disconnectButton');
});
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('http://N1-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
...
$(document).on('click', '#create', function(e){
    e.preventDefault();
    $.post('http://N1-multicharacter/createNewCharacter', JSON.stringify({
    ...
    N1MultiCharacters.fadeOutDown('.character-register', '125%', 400);
    refreshCharacters()
});

$(document).on('click', '#accept-delete', function(e){
    $.post('http://N1-multicharacter/removeCharacter', JSON.stringify({
...
$(document).on('click', '#play', function(e) {
    ...
    if (selectedChar !== null) {
        if (charData !== "") {
            $.post('http://N1-multicharacter/selectCharacter', JSON.stringify({
                cData: $(selectedChar).data('cData')
            }));
            // N1MultiCharacters.fadeInDown('.welcomescreen', WelcomePercentage, 400);
            // N1MultiCharacters.fadeInDown('.server-log', '25%', 400);
            setTimeout(function(){
                N1MultiCharacters.fadeOutDown('.characters-list', "-40%", 400);
                N1MultiCharacters.fadeOutDown('.character-info', "-40%", 400);
                N1MultiCharacters.resetAll();
            }, 1500);
        } else {
            $('.characters-list').css("filter", "blur(2px)")
            $('.character-info').css("filter", "blur(2px)")
            N1MultiCharacters.fadeInDown('.character-register', '25%', 400);
        }
    }
});
...
N1MultiCharacters.fadeOutUp = function(element, time) {
    $(element).css({"display":"block"}).animate({top: "-80.5%",}, time, function(){
        $(element).css({"display":"none"});
    });
}

N1MultiCharacters.fadeOutDown = function(element, percent, time) {
...
N1MultiCharacters.fadeInDown = function(element, percent, time) {
    $(element).css({"display":"block"}).animate({top: percent,}, time);
}

N1MultiCharacters.resetAll = function() {
