...
function init($param) {
    ...
    if($param!=1)
        $("#container > p").append("<br><h 4at the right time</h4>");
    else
        $("#container > p").append("<br><h 4at the right time</h4>");
    $('.time').addClass('done');
}
...
$(document).ready(function() {
    ...
    window.addEventListener('click',function() {
        ...
        if (distance < HitDistance) {
            ...
            MainDigits--;
            $.post("http://N1-ifruitstore/thermiteclick");
...
Thermite.Failed = function() {
    if (active) $.post("http://N1-ifruitstore/thermitefailed");
    active = false;
    Thermite.Close();
}

Thermite.Success = function() {
    if (active) $.post("http://N1-ifruitstore/thermitesuccess");
    active = false;
    Thermite.Close();
}
...
Thermite.Close = function() {
    ...
    $.post('http://N1-ifruitstore/closethermite');
}