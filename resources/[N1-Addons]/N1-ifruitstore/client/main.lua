                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z, true) < 1.0 then
                    if not Config.Locations["thermite"].isDone then 
                        if not requiredItemsShowed then
                            requiredItems = {
                                [1] = {name = N1Core.Shared.Items["thermite"]["name"], image = N1Core.Shared.Items["thermite"]["image"]},
                            }
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["safe"].x, Config.Locations["safe"].y,Config.Locations["safe"].z, true) < 1.0 then
                if not Config.Locations["safe"].isDone then 
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        N1Core.Functions.TriggerCallback('N1-storerobbery:server:getPadlockCombination', function(combination)
                            TriggerServerEvent("N1-ifruitstore:server:SetSafeStatus", "isBusy", true)
                ...
                if dist < 1.0 then
                    ...
                    if dist < 0.6 then
                        if not requiredItemsShowed2 then
                            requiredItems = {
                                [1] = {name = N1Core.Shared.Items["advancedlockpick"]["name"], image = N1Core.Shared.Items["advancedlockpick"]["image"]},
                            }
                                    ...
                                    if Config.Locations["thermite"].isDone then 
                                        N1Core.Functions.TriggerCallback('N1-radio:server:GetItem', function(hasItem)
                                            if hasItem then
                                                ...
                                                TriggerEvent("N1-lockpick:client:openLockpick", lockpickDone)
                                            else
                                                N1Core.Functions.Notify("You're missing an advanced lockpick..", "error")
                                            end
                                        end, "advancedlockpick")
                                    else
                                        N1Core.Functions.Notify("Security stil active..", "error")
                                    end
                                else
                                    N1Core.Functions.Notify("Not enough police..", "error")
                                end
                            end
                        end
                    end
...
function lockpickDone(success)
    ...
    if success then
        ...
        if math.random(1, 100) <= 40 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
            N1Core.Functions.Notify("You ripped your glove.")
        end
        if math.random(1, 100) <= 10 then
            TriggerServerEvent("N1Core:Server:RemoveItem", "advancedlockpick", 1)
            TriggerEvent('inventory:client:ItemBox', N1Core.Shared.Items["advancedlockpick"], "remove")
        end
    end
end
...
function GrabItem(spot)
    ...
    N1Core.Functions.Progressbar("grab_ifruititem", "Disconnect item..", 5000, false, true, {
        ...
        TriggerServerEvent('N1-ifruitstore:server:setSpotState', "isDone", true, spot)
        TriggerServerEvent('N1-ifruitstore:server:setSpotState', "isBusy", false, spot)
        TriggerServerEvent('N1-ifruitstore:server:itemReward', spot)
        TriggerServerEvent('N1-ifruitstore:server:PoliceAlertMessage', 'People try to steal items at the iFruit store!', pos, true)
    end, function() -- Cancel
        StopAnimTask(GetPlayerPed(-1), "anim@gangops@facility@servers@", "hotwire", 1.0)
        TriggerServerEvent('N1-jewellery:server:setSpotState', "isBusy", false, spot)
    ...
    if usingSafe then
        if won then
            if not Config.Locations["safe"].isDone then
                ...
                TriggerServerEvent("N1-ifruitstore:server:SafeReward")
                TriggerServerEvent("N1-ifruitstore:server:SetSafeStatus", "isBusy", false)
                TriggerServerEvent("N1-ifruitstore:server:SetSafeStatus", "isDone", false)
                takeAnim()
            end
        end
    end
...
AddEventHandler('N1Core:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    TriggerServerEvent("N1-ifruitstore:server:LoadLocationList")
end)
...
RegisterNetEvent('N1-ifruitstore:client:LoadList')
AddEventHandler('N1-ifruitstore:client:LoadList', function(list)
    ...
    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z, true) < 1.0 then
        if CurrentCops >= 0 then
            ...
            if requiredItemsShowed then
                ...
                requiredItemsShowed = false
                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                TriggerServerEvent("N1-ifruitstore:server:SetThermiteStatus", "isBusy", true)
...
RegisterNetEvent('N1-ifruitstore:client:setSpotState')
AddEventHandler('N1-ifruitstore:client:setSpotState', function(stateType, state, spot)
...
RegisterNetEvent('N1-ifruitstore:client:SetSafeStatus')
AddEventHandler('N1-ifruitstore:client:SetSafeStatus', function(stateType, state)
    if stateType == "isBusy" then
...
RegisterNetEvent('N1-ifruitstore:client:SetThermiteStatus')
AddEventHandler('N1-ifruitstore:client:SetThermiteStatus', function(stateType, state)
    if stateType == "isBusy" then
...
RegisterNetEvent('N1-ifruitstore:client:PoliceAlertMessage')
AddEventHandler('N1-ifruitstore:client:PoliceAlertMessage', function(msg, coords, blip)
    if blip then
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent("N1-ifruitstore:server:SetThermiteStatus", "isBusy", false)
    TriggerServerEvent("N1Core:Server:RemoveItem", "thermite", 1)
    TriggerEvent('inventory:client:ItemBox', N1Core.Shared.Items["thermite"], "remove")
end)

RegisterNUICallback('thermitesuccess', function()
    N1Core.Functions.Notify("De zekeringen zijn kapot", "success")
    local pos = GetEntityCoords(GetPlayerPed(-1))
    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z, true) < 1.0 then
        TriggerServerEvent("N1-ifruitstore:server:SetThermiteStatus", "isDone", true)
        TriggerServerEvent("N1-ifruitstore:server:SetThermiteStatus", "isBusy", false)
    end
end)
