...
RegisterNetEvent('N1-policealerts:ToggleDuty')
AddEventHandler('N1-policealerts:ToggleDuty', function(Duty)
    PlayerJob.onduty = Duty
end)

RegisterNetEvent('N1-policealerts:client:AddPoliceAlert')
AddEventHandler('N1-policealerts:client:AddPoliceAlert', function(data, forBoth)
    if forBoth then
...
RegisterNUICallback('SetWaypoint', function(data)
    ...
    N1Core.Functions.Notify('GPS ingesteld!')
    SetNuiFocus(false, false)
    SetNuiFocusKeepInput(false, false)
    MouseActive = false
end)
