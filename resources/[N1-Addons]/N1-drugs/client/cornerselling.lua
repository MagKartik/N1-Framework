...
RegisterNetEvent('N1-drugs:client:cornerselling')
AddEventHandler('N1-drugs:client:cornerselling', function(data)
    N1Core.Functions.TriggerCallback('N1-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            availableDrugs = result

            if not cornerselling then
                cornerselling = true
                N1Core.Functions.Notify('Flicked the cornerselling switch.. yeet')
                startLocation = GetEntityCoords(GetPlayerPed(-1))
                -- TaskStartScenarioInPlace(GetPlayerPed(-1), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
            else
                cornerselling = false
                N1Core.Functions.Notify('Corner selling position disabled .. yeet')
                -- ClearPedTasks(GetPlayerPed(-1))
            end
        else
            N1Core.Functions.Notify('You aren\'t carrying any weed with you..', 'error')
        end
    end)
end)

function toFarAway()
    N1Core.Functions.Notify('You are moving too much, start over again!', 'error')
    cornerselling = false
    hasTarget = false
    busySelling = false
    startLocation = nil
            ...
            if IsEntityDead(stealingPed) then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, pedpos.x, pedpos.y, pedpos.z, true) < 1.5) then
                    ...
                    if IsControlJustReleased(0, Keys["E"]) then
                        ...
                        Citizen.Wait(2000)
                        ClearPedTasks(GetPlayerPed(-1))
                        TriggerServerEvent("N1Core:Server:AddItem", stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', N1Core.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
...
RegisterNetEvent('N1-drugs:client:refreshAvailableDrugs')
AddEventHandler('N1-drugs:client:refreshAvailableDrugs', function(items)
...
function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('N1-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                N1Core.Functions.Notify('You have been robbed and lost '..bagAmount..' bags(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 then
                    N1Core.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' for ₹'..randomPrice..'? / ~g~G~w~ Decline offer')
                    if IsControlJustPressed(0, Keys["E"]) then
                        N1Core.Functions.Notify('Offer accepted!', 'success')
                        TriggerServerEvent('N1-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, Keys["G"]) then
                        N1Core.Functions.Notify('Offer canceled!', 'error')
                        hasTarget = false
