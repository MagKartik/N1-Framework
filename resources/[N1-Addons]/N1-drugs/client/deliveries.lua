...
AddEventHandler('N1Core:Client:OnPlayerLoaded', function()
    N1Core.Functions.TriggerCallback('N1-drugs:server:RequestConfig', function(DealerConfig)
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        N1Core.Functions.Progressbar("hospital_revive", "Put person back on their feet..", 5000, false, true, {
                                            ...
                                            N1Core.Functions.Notify("You have helped a person!")
                                        ...
                                        N1Core.Functions.Notify("There is no one nearby..", "error")
                                    end
                                else
...
function buyDealerStuff()
    ...
    for k, v in pairs(Config.Dealers[currentDealer]["products"]) do
        if N1Core.Functions.GetPlayerData().metadata["dealerrep"] >= Config.Dealers[currentDealer]["products"][k].minrep then
...
function knockDoorAnim(home)
    ...
    local myData = N1Core.Functions.GetPlayerData()
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        N1Core.Functions.Notify('It looks like no one is home..', 'error', 3500)
    end
end

RegisterNetEvent('N1-drugs:client:updateDealerItems')
AddEventHandler('N1-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('N1-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('N1-drugs:client:setDealerItems')
AddEventHandler('N1-drugs:client:setDealerItems', function(itemData, amount, dealer)
...
function requestDelivery()
    ...
    TriggerServerEvent('N1-drugs:server:giveDeliveryItems', amount)
    SetTimeout(2000, function()
        TriggerServerEvent('N1-phone:server:sendNewMail', {
            sender = Config.Dealers[currentDealer]["name"],
            subject = "Aflever Locatie",
            message = "Here is all info about the delivery, <br>Items: <br> "..amount.."x "..N1Core.Shared.Items[waitingDelivery["itemData"]["item"]]["label"].."<br><br> be in time!!",
            button = {
                enabled = true,
                buttonEvent = "N1-drugs:client:setLocation",
...
function randomDeliveryItemOnRep()
    local ped = GetPlayerPed(-1)
    local myRep = N1Core.Functions.GetPlayerData().metadata["dealerrep"]
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    N1Core.Functions.Notify('The route to the delivery location has been set on your map.', 'success');
end

RegisterNetEvent('N1-drugs:client:setLocation')
AddEventHandler('N1-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        activeDelivery = locationData
    else
        setMapBlip(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"])
        N1Core.Functions.Notify('You still have an open delivery...')
        return
    end
        ...
        while true do
            ...
            if activeDelivery ~= nil then
                ...
                if dist < 15 then
                    ...
                    if dist < 1.5 then
                        DrawText3D(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"], activeDelivery["coords"]["z"], '[E] '..activeDelivery["amount"]..'x '..N1Core.Shared.Items[activeDelivery["itemData"]["item"]]["label"]..' deliver.')
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        checkPedDistance()
        N1Core.Functions.Progressbar("work_dropbox", "Delivering products..", 3500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function() -- Done
            TriggerServerEvent('N1-drugs:server:succesDelivery', activeDelivery, true)
        end, function() -- Cancel
            ClearPedTasks(GetPlayerPed(-1))
            N1Core.Functions.Notify("Canceled..", "error")
        end)
    else
        TriggerServerEvent('N1-drugs:server:succesDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
function checkPedDistance()
    ...
    local closestPed, closestDistance = N1Core.Functions.GetClosestPed(coords, PlayerPeds)
...
function doPoliceAlert()
    ...
    TriggerServerEvent('N1-drugs:server:callCops', streetLabel, pos)
end

RegisterNetEvent('N1-drugs:client:robberyCall')
AddEventHandler('N1-drugs:client:robberyCall', function(msg, streetLabel, coords)
...
RegisterNetEvent('N1-drugs:client:sendDeliveryMail')
AddEventHandler('N1-drugs:client:sendDeliveryMail', function(type, deliveryData)
...
RegisterNetEvent('N1-drugs:client:CreateDealer')
AddEventHandler('N1-drugs:client:CreateDealer', function(dealerName, minTime, maxTime)
    local ped = GetPlayerPed(-1)
    ...
    TriggerServerEvent('N1-drugs:server:CreateDealer', DealerData)
end)

RegisterNetEvent('N1-drugs:client:RefreshDealers')
AddEventHandler('N1-drugs:client:RefreshDealers', function(DealerData)
    Config.Dealers = DealerData
end)

RegisterNetEvent('N1-drugs:client:GotoDealer')
AddEventHandler('N1-drugs:client:GotoDealer', function(DealerData)
    local ped = GetPlayerPed(-1)

    SetEntityCoords(ped, DealerData["coords"]["x"], DealerData["coords"]["y"], DealerData["coords"]["z"])
    N1Core.Functions.Notify('You have been teleported : '.. DealerData["name"] .. ' Good luck!', 'success')
end)