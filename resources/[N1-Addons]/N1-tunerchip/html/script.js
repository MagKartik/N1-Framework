...
$(document).on('click', '#save', function(){
    $.post('http://N1-tunerchip/save', JSON.stringify({
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('http://N1-tunerchip/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('http://N1-tunerchip/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('http://N1-tunerchip/SetStancer', JSON.stringify({
...
N1Tuner.Open = function() {
    $.post('http://N1-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
N1Tuner.Close = function() {
    $('.container').fadeOut(250);
    $.post('http://N1-tunerchip/exit');
}