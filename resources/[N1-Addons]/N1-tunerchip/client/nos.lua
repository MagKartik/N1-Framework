...
AddEventHandler('N1Core:Client:OnPlayerLoaded', function()
    N1Core.Functions.TriggerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
...
AddEventHandler('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            N1Core.Functions.Progressbar("use_nos", "NOS is getting connected.", 1000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
            }, {}, {}, {}, function() -- Done
                TriggerEvent("inventory:client:ItemBox", N1Core.Shared.Items['nitrous'], "remove")
                TriggerServerEvent("N1Core:Server:RemoveItem", 'nitrous', 1)
                local CurrentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
                local Plate = GetVehicleNumberPlateText(CurrentVehicle)
                TriggerServerEvent('nitrous:server:LoadNitrous', Plate)
            end)
        else
            N1Core.Functions.Notify('You are not in vehicle.', 'error')
        end
    else
        N1Core.Functions.Notify('You already have NOS active..', 'error')
    end
end)
        ...
        if IsInVehicle then
            ...
            if VehicleNitrous[Plate] ~= nil then
                if VehicleNitrous[Plate].hasnitro then
                        ...
                        Citizen.CreateThread(function()
                            while NitrousActivated do
                                if VehicleNitrous[Plate].level - 1 ~= 0 then
                                    ...
                                    TriggerEvent('N1-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, true)
                                else
                    ...
                    if IsControlJustReleased(0, Keys["LEFTCTRL"]) then
                        if NitrousActivated then
                            ...
                            StopScreenEffect("RaceTurbo")
                            TriggerEvent('N1-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, false)
                            NitrousActivated = false
                        end
                    end
                end
                ...
                if not nosupdated then
                    TriggerEvent('N1-hud:client:UpdateNitrous', false, nil, false)
                    nosupdated = true
                end
            end
        else
