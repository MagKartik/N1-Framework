...
RegisterNUICallback('save', function(data)
    N1Core.Functions.TriggerCallback('N1-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = GetPlayerPed(-1)
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            N1Core.Functions.Notify('Tjoenertjip v1.05: Vehicle changed!', 'error')

            TriggerServerEvent('N1-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end)

RegisterNetEvent('N1-tunerchip:server:TuneStatus')
AddEventHandler('N1-tunerchip:server:TuneStatus', function()
    local ped = GetPlayerPed(-1)
    ...
    N1Core.Functions.TriggerCallback('N1-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    N1Core.Functions.Notify('Tjoenertjip v1.05: Vehicle is resetted!', 'error')
end)

RegisterNetEvent('N1-tunerchip:client:openChip')
AddEventHandler('N1-tunerchip:client:openChip', function()
    local ped = GetPlayerPed(-1)
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        N1Core.Functions.Progressbar("connect_laptop", "Tunerlaptop is getting connected..", 2000, false, true, {
