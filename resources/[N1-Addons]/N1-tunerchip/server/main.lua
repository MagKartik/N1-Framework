...
N1Core.Functions.CreateUseableItem("tunerlaptop", function(source, item)
    local src = source

    TriggerClientEvent('N1-tunerchip:client:openChip', src)
end)

RegisterServerEvent('N1-tunerchip:server:TuneStatus')
AddEventHandler('N1-tunerchip:server:TuneStatus', function(plate, bool)
    if bool then
...
N1Core.Functions.CreateCallback('N1-tunerchip:server:HasChip', function(source, cb)
    local src = source
...
N1Core.Functions.CreateCallback('N1-tunerchip:server:GetStatus', function(source, cb, plate)
    cb(tunedVehicles[plate])
end)

N1Core.Functions.CreateUseableItem("nitrous", function(source, item)
