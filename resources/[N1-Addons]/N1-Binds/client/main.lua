...
RegisterNetEvent('N1-commandbinding:client:openUI')
AddEventHandler('N1-commandbinding:client:openUI', function()
    openBindingMenu()
end)
        ...
        if isLoggedIn then
            for k, v in pairs(availableKeys) do
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    local keyMeta = N1Core.Functions.GetPlayerData().metadata["commandbinds"]
                    local args = {}
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            TriggerServerEvent('N1Core:CallCommand', keyMeta[v]["command"], args)
                            keyPressed = true
                        else
                            N1Core.Functions.Notify('Nothings is binded on key ['..v..'] , /binds to bind a command', 'primary', 4000)
                        end
                    else
                        N1Core.Functions.Notify('You have not binded any commands, /binds to bind a command', 'primary', 4000)
                    end
                end
            end
...
RegisterNUICallback('save', function(data)
    ...
    N1Core.Functions.Notify('Commandbindings saved!', 'success')

    TriggerServerEvent('N1-commandbinding:server:setKeyMeta', keyData)
end)