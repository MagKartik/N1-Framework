...
Citizen.CreateThread(function()
    while true do
        ...
        if N1Core ~= nil then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), true), -1) == GetPlayerPed(-1) then
                ...
                if LastVehicle ~= GetVehiclePedIsIn(GetPlayerPed(-1), false) then
                    N1Core.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
                        if result then
                            HasKey = true
        ...
        if not HasKey and IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and N1Core ~= nil and not IsHotwiring then
            local veh = GetVehiclePedIsIn(GetPlayerPed(-1), false)
            SetVehicleEngineOn(veh, false, false, true)
            --[[local veh = GetVehiclePedIsIn(GetPlayerPed(-1), false)
            local vehpos = GetOffsetFromEntityInWorldCoords(veh, 0, 1.5, 0.5)
            N1Core.Functions.DrawText3D(vehpos.x, vehpos.y, vehpos.z, "~g~H~w~ - Hotwire")
            SetVehicleEngineOn(veh, false, false, true)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(7)
        if not IsRobbing and isLoggedIn and N1Core ~= nil then
            if GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= nil and GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= 0 then
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) then
                    if IsEntityDead(driver) then
                        IsRobbing = true
                        N1Core.Functions.Progressbar("rob_keys", "Grabing keysn..", 3000, false, true, {}, {}, {}, {}, function() -- Done
            ...
            --[[if N1Core.Functions.GetPlayerData().job.name ~= "police" then
...
AddEventHandler('vehiclekeys:client:SetOwner', function(plate)
    ...
    HasKey = true
    --N1Core.Functions.Notify('You picked the keys of the vehicle', 'success', 3500)
end)
...
function LockVehicle()
    local veh = N1Core.Functions.GetClosestVehicle()
    ...
    if veh ~= nil and GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 7.5 then
        N1Core.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
            if result then
                if HasKey then
                    ...
                    if vehLockStatus == 1 then
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 2)then
                            N1Core.Functions.Notify("Voertuig vergrendeld!")
                        else
                            N1Core.Functions.Notify("Something went wrong whit the locking system!")
                        end
                    else
                        Citizen.Wait(750)
                        ClearPedTasks(GetPlayerPed(-1))
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 1)then
                            N1Core.Functions.Notify("Voertuig ontgrendeld!")
                        else
                            N1Core.Functions.Notify("Something went wrong whit the locking system!")
                        end
                    end
                ...
                N1Core.Functions.Notify('You dont have the keys of the vehicle..', 'error')
            end
        end, plate)
    end
end
    ...
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 1.5 then
            ...
            if (vehLockStatus > 1) then
                ...
                N1Core.Functions.Progressbar("lockpick_vehicledoor", "breaking the door open..", lockpickTime, false, true, {
                    ...
                    if math.random(1, 100) <= 90 then
                        N1Core.Functions.Notify("Door open!")
                        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "unlock", 0.3)
                ...
                Skillbar.Start({
                    ...
                    if IsHotwiring then
                        if SucceededAttempts + 1 >= NeededAttempts then
                            ...
                            N1Core.Functions.Notify("Lockpicking succeeded!")
                            HasKey = true
                            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
                            IsHotwiring = false
                            FailedAttemps = 0
                            SucceededAttempts = 0
                            NeededAttempts = 0
                            TriggerServerEvent('N1-hud:Server:GainStress', math.random(2, 4))
                        else
                            if vehicle ~= nil and vehicle ~= 0 then
                                ...
                                N1Core.Functions.Notify("You have to be in the vehicle", "error")
                                IsHotwiring = false
                                FailedAttemps = FailedAttemps + 1
                                local c = math.random(2)
                                local o = math.random(2)
                                if c == o then
                                    TriggerServerEvent('N1-hud:Server:GainStress', math.random(1, 4))
                                end
                            end
                        end
                    end
                    ...
                    if IsHotwiring then
                        ...
                        HasKey = false
                        SetVehicleEngineOn(vehicle, false, false, true)
                        N1Core.Functions.Notify("Lockpicking failed!", "error")
                        IsHotwiring = false
                        FailedAttemps = FailedAttemps + 1
                        local c = math.random(2)
                        local o = math.random(2)
...
function Hotwire()
    if not HasKey then 
        ...
        PoliceCall()
        N1Core.Functions.Progressbar("hotwire_vehicle", "Engaging the ignition switch", hotwireTime, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            if (math.random(0, 100) < 10) then
                HasKey = true
                N1Core.Functions.Notify("Hotwire succeeded!")
            else
                HasKey = false
                SetVehicleEngineOn(veh, false, false, true)
                N1Core.Functions.Notify("Hotwire failed!", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        ...
        if math.random(1, 100) <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    ...
                    if N1Core.Shared.VehicleModels[modelName] ~= nil then
                        Name = N1Core.Shared.Vehicles[N1Core.Shared.VehicleModels[modelName]["model"]]["brand"] .. ' ' .. N1Core.Shared.Vehicles[N1Core.Shared.VehicleModels[modelName]["model"]]["name"]
                    else
                        Name = "Unknown"
                    end
