...
RegisterServerEvent('N1-storerobbery:server:takeMoney')
AddEventHandler('N1-storerobbery:server:takeMoney', function(register, isDone)
    local src   = source
...
RegisterServerEvent('N1-storerobbery:server:setRegisterStatus')
AddEventHandler('N1-storerobbery:server:setRegisterStatus', function(register)
    TriggerClientEvent('N1-storerobbery:client:setRegisterStatus', -1, register, true)
    Config.Registers[register].robbed   = true
    Config.Registers[register].time     = Config.resetTime
end)

RegisterServerEvent('N1-storerobbery:server:setSafeStatus')
AddEventHandler('N1-storerobbery:server:setSafeStatus', function(safe)
    TriggerClientEvent('N1-storerobbery:client:setSafeStatus', -1, safe, true)
    Config.Safes[safe].robbed = true

    SetTimeout(math.random(40, 80) * (60 * 1000), function()
        TriggerClientEvent('N1-storerobbery:client:setSafeStatus', -1, safe, false)
        Config.Safes[safe].robbed = false
    end)
end)

RegisterServerEvent('N1-storerobbery:server:SafeReward')
AddEventHandler('N1-storerobbery:server:SafeReward', function(safe)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    ...
    if luck <= 10 then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items["rolex"], "add")
        if luck == odd then
            Citizen.Wait(500)
...
RegisterServerEvent('N1-storerobbery:server:callCops')
AddEventHandler('N1-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId = 4
    if type == "safe" then
    ...
    TriggerClientEvent("N1-storerobbery:client:robberyCall", -1, type, safe, streetLabel, coords)
    ...
    while true do
        for k, v in pairs(Config.Registers) do
            if Config.Registers[k].time > 0 and (Config.Registers[k].time - Config.tickInterval) >= 0 then
                ...
                TriggerClientEvent('N1-storerobbery:client:setRegisterStatus', -1, k, false)
            end
        end
...
N1Core.Functions.CreateCallback('N1-storerobbery:server:isCombinationRight', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

N1Core.Functions.CreateCallback('N1-storerobbery:server:getPadlockCombination', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

N1Core.Functions.CreateCallback('N1-storerobbery:server:getRegisterStatus', function(source, cb)
    cb(Config.Registers)
end)

N1Core.Functions.CreateCallback('N1-storerobbery:server:getSafeStatus', function(source, cb)
    cb(Config.Safes)
end)