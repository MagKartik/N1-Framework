...
AddEventHandler('inventory:client:CheckOpenState', function(type, id, label)
    local name = N1Core.Shared.SplitStr(label, "-")[2]
    if type == "stash" then
        ...
        if showTrunkPos and not inInventory then
            ...
            if vehicle ~= 0 and vehicle ~= nil then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    ...
                    N1Core.Functions.DrawText3D(drawpos.x, drawpos.y, drawpos.z, "Trunk")
                        ...
                        if vehicle ~= 0 and vehicle ~= nil then
                            ...
                            if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, trunkpos) < 2.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                if GetVehicleDoorLockStatus(vehicle) < 2 then
                                    ...
                                    N1Core.Functions.Notify("Vehicle is locked..", "error")
                                    return
                                end
                            else
                                CurrentVehicle = nil
    ...
    if bool then
        for k, v in pairs(items) do
            table.insert(itemTable, {
                item = items[k].name,
                label = N1Core.Shared.Items[items[k].name]["label"],
                image = items[k].image,
            })
        end
    end
...
RegisterNUICallback('Notify', function(data, cb)
    N1Core.Functions.Notify(data.message, data.type)
end)
...
AddEventHandler("inventory:client:OpenInventory", function(PlayerAmmo, inventory, other)
    if not IsEntityDead(GetPlayerPed(-1)) then
        ...
        SendNUIMessage({
            ...
            other = other,
            maxweight = N1Core.Config.Player.MaxWeight,
            Ammo = PlayerAmmo,
...
AddEventHandler("inventory:client:UpdatePlayerInventory", function(isError)
    SendNUIMessage({
        action = "update",
        inventory = N1Core.Functions.GetPlayerData().items,
        maxweight = N1Core.Config.Player.MaxWeight,
        slots = MaxInventorySlots,
        error = isError,
    })
...
AddEventHandler("inventory:client:CraftItems", function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    N1Core.Functions.Progressbar("repair_vehicle", "Crafting..", (math.random(2000, 5000) * amount), false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', N1Core.Shared.Items[itemName], 'add')
        isCrafting = false
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
...
AddEventHandler("inventory:client:PickupSnowballs", function()
    ...
    N1Core.Functions.Progressbar("pickupsnowball", "Sneeuwballen oprapen..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerServerEvent('N1Core:Server:AddItem', "snowball", 1)
        TriggerEvent('inventory:client:ItemBox', N1Core.Shared.Items["snowball"], "add")
    end, function() -- Cancel
        ClearPedTasks(GetPlayerPed(-1))
...
AddEventHandler("inventory:client:UseWeapon", function(weaponData, shootbool)
    ...
    elseif weaponName == "weapon_stickybomb" then
        ...
        TriggerServerEvent('N1Core:Server:RemoveItem', weaponName, 1)
    ...
    elseif weaponName == "weapon_snowball" then
        ...
        TriggerEvent('weapons:client:SetCurrentWeapon', weaponData, shootbool)
        N1Core.Functions.TriggerCallback("weapon:server:GetWeaponAmmo", function(result)
...
RegisterNUICallback('GetWeaponData', function(data, cb)
    local data = {
        WeaponData = N1Core.Shared.Items[data.weapon],
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    local WeaponData = N1Core.Shared.Items[data.WeaponData.name]
    local Attachment = WeaponAttachments[WeaponData.name:upper()][data.AttachmentData.attachment]
    
    N1Core.Functions.TriggerCallback('weapons:server:RemoveAttachment', function(NewAttachments)
...
RegisterNUICallback('getCombineItem', function(data, cb)
    cb(N1Core.Shared.Items[data.item])
end)
...
RegisterNUICallback("combineItem", function(data)
    ...
    TriggerEvent('inventory:client:ItemBox', N1Core.Shared.Items[data.reward], 'add')
end)
...
RegisterNUICallback('combineWithAnim', function(data)
    ...
    N1Core.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', N1Core.Shared.Items[combineData.reward], 'add')
    end, function() -- Cancel
        StopAnimTask(GetPlayerPed(-1), aDict, aLib, 1.0)
        N1Core.Functions.Notify("Mislukt!", "error")
    end)
end)
...
function ToggleHotbar(toggle)
    local HotbarItems = {
        [1] = N1Core.Functions.GetPlayerData().items[1],
        [2] = N1Core.Functions.GetPlayerData().items[2],
        [3] = N1Core.Functions.GetPlayerData().items[3],
        [4] = N1Core.Functions.GetPlayerData().items[4],
        [5] = N1Core.Functions.GetPlayerData().items[5],
        [41] = N1Core.Functions.GetPlayerData().items[41],
    } 
