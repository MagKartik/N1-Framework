...
RegisterServerEvent("N1-pawnshop:server:sellPawnItems")
AddEventHandler("N1-pawnshop:server:sellPawnItems", function()
    local src = source
    local price = 0
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        TriggerClientEvent('N1Core:Notify', src, "You have sold your items")
    end
end)

RegisterServerEvent("N1-pawnshop:server:sellHardwarePawnItems")
AddEventHandler("N1-pawnshop:server:sellHardwarePawnItems", function()
    local src = source
    local price = 0
    local totalAmount = 0
    local Player = N1Core.Functions.GetPlayer(src)
...
RegisterServerEvent("N1-pawnshop:server:getGoldBars")
AddEventHandler("N1-pawnshop:server:getGoldBars", function()
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    if GoldBarsAmount > 0 then
        if Player.Functions.AddItem("goldbar", GoldBarsAmount) then
            ...
            Config.MeltTime = 300
            TriggerClientEvent("N1-pawnshop:client:SetTakeState", -1, false)
        else
            TriggerClientEvent('N1Core:Notify', src, "You dont have any space in your inventory", "error")
        end
    end
end)

RegisterServerEvent("N1-pawnshop:server:sellGold")
AddEventHandler("N1-pawnshop:server:sellGold", function()
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    local price = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        TriggerClientEvent('N1Core:Notify', src, "You have sold your item's")
    end
end)

RegisterServerEvent("N1-pawnshop:server:meltItems")
AddEventHandler("N1-pawnshop:server:meltItems", function()
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    local goldbars = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
                if MeltItems[Player.PlayerData.items[k].name] ~= nil then 
                    ...
                    if amount < 1 then
                        TriggerClientEvent('N1Core:Notify', src, "You dont have enough " .. Player.PlayerData.items[k].label, "error")
                    else
        ...
        if goldbars > 0 then
            ...
            TriggerClientEvent('N1-pawnshop:client:startMelting', -1)
            ...
            Citizen.CreateThread(function()
                while Config.IsMelting do
                    ...
                    if Config.MeltTime <= 0 then
                        ...
                        Config.CanTake = true
                        Config.MeltTime = 300
                        TriggerClientEvent('N1-pawnshop:client:SetTakeState', -1, true)
                    end
                    Citizen.Wait(1000)
                end
            end)
...
N1Core.Functions.CreateCallback('N1-pawnshop:server:getSellPrice', function(source, cb)
    local retval = 0
...
N1Core.Functions.CreateCallback('N1-pawnshop:melting:server:GetConfig', function(source, cb)
    cb(Config.IsMelting, Config.MeltTime, Config.CanTake)
end)

N1Core.Functions.CreateCallback('N1-pawnshop:server:getSellHardwarePrice', function(source, cb)
    local retval = 0
    local amount = 0
    local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
...
N1Core.Functions.CreateCallback('N1-pawnshop:server:hasGold', function(source, cb)
	local retval = false
    local Player = N1Core.Functions.GetPlayer(source)
