...
AddEventHandler("N1Core:Client:OnPlayerLoaded", function()
    N1Core.Functions.TriggerCallback('N1-pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            TriggerServerEvent("N1-pawnshop:server:getGoldBars")
                        end
                    else
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then 
                            ...
                            ScrapAnim(1000)
                            N1Core.Functions.Progressbar("drop_golden_stuff", "Picking up items..", 1000, false, true, {
                                ...
                                if not Config.IsMelting then
                                    ...
                                    TriggerServerEvent("N1-pawnshop:server:meltItems")
                                end
                            end)
                        end
                    end
                ...
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            ...
                            N1Core.Functions.Progressbar("sell_gold", "Sell gold..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                TriggerServerEvent('N1-pawnshop:server:sellGold')
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(GetPlayerPed(-1))
                                N1Core.Functions.Notify("Process canceled..", "error")
                            end)
                        end
                    else
...
function HasPlayerGold()
	local retval = false
	N1Core.Functions.TriggerCallback('N1-pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
...
RegisterNetEvent('N1-pawnshop:client:startMelting')
AddEventHandler('N1-pawnshop:client:startMelting', function()
    if not Config.IsMelting then
...
RegisterNetEvent('N1-pawnshop:client:SetTakeState')
AddEventHandler('N1-pawnshop:client:SetTakeState', function(state)
