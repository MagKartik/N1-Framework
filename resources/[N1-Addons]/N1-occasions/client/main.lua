            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        N1Core.Functions.TriggerCallback('N1-occasions:server:getVehicles', function(vehicles)
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, N1Core.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Price: ₹'..Config.OccasionSlots[i]["price"]..',-')
                                if Config.OccasionSlots[i]["owner"] == N1Core.Functions.GetPlayerData().citizenid then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    N1Core.Functions.TriggerCallback('N1-occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
                                ...
                                if IsDisabledControlJustPressed(0, Keys["7"]) then
                                    isConfirming = false
                                    currentVehicle = i
                                    TriggerServerEvent("N1-occasions:server:ReturnVehicle", Config.OccasionSlots[i])
                                end
                        ...
                        if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            N1Core.Functions.TriggerCallback('N1-garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    ...
                                    N1Core.Functions.Notify('This is not your vehicle?', 'error', 3500)
                                end
                            end, VehiclePlate)
                        end
                    end
...
function spawnOccasionsVehicles(vehicles)
    ...
    if vehicles ~= nil then
        for i = 1, #vehicles, 1 do
            ...
            N1Core.Functions.SetVehicleProperties(oSlot[i]["occasionid"], json.decode(oSlot[i]["mods"]))
...
function openSellContract(bool)
    ...
    SendNUIMessage({
        action = "sellVehicle",
        pData = N1Core.Functions.GetPlayerData(),
...
RegisterNUICallback('error', function(data)
    N1Core.Functions.Notify(data.message, 'error')
end)

RegisterNUICallback('buyVehicle', function()
    local vehData = Config.OccasionSlots[currentVehicle]
    TriggerServerEvent('N1-occasions:server:buyVehicle', vehData)
end)

DoScreenFadeIn(250)

RegisterNetEvent('N1-occasions:client:BuyFinished')
AddEventHandler('N1-occasions:client:BuyFinished', function(model, plate, mods)
    ...
    N1Core.Functions.SpawnVehicle(model, function(veh)
        ...
        N1Core.Functions.Notify("Vehicle bought", "success", 2500)
        TriggerEvent("vehiclekeys:client:SetOwner", plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        N1Core.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('N1-occasions:client:ReturnOwnedVehicle')
AddEventHandler('N1-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    N1Core.Functions.SpawnVehicle(vehdata.model, function(veh)
        SetVehicleNumberPlateText(veh, vehdata.plate)
        SetEntityHeading(veh, Config.BuyVehicle.h)
        TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
        exports['LegacyFuel']:SetFuel(veh, 100)
        N1Core.Functions.Notify("Your vehicle recieved ..")
...
RegisterNUICallback('sellVehicle', function(data)
    ...
    vehicleData.model = N1Core.Shared.VehicleModels[GetEntityModel(vehicleData.ent)]["model"]
    vehicleData.plate = GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1)))
    vehicleData.mods = N1Core.Functions.GetVehicleProperties(vehicleData.ent)
    vehicleData.desc = data.desc

    TriggerServerEvent('N1-occasions:server:sellVehicle', data.price, vehicleData)
...
function sellVehicleWait(price)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    N1Core.Functions.DeleteVehicle(GetVehiclePedIsIn(GetPlayerPed(-1)))
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    N1Core.Functions.Notify('Your car is for sale for ₹'..price..',-', 'success')
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end

RegisterNetEvent('N1-occasion:client:refreshVehicles')
AddEventHandler('N1-occasion:client:refreshVehicles', function()
    if inRange then
        N1Core.Functions.TriggerCallback('N1-occasions:server:getVehicles', function(vehicles)
            occasionVehicles = vehicles
            despawnOccasionsVehicles()
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Quino's RV Sales")
