...
$(document).ready(function(){
    ...
    window.addEventListener('message', function(event){
        ...
        if (eventData.action == "sellVehicle") {
            ...
            N1Occasions.setupSellContract(eventData)
        }

        if (eventData.action == "buyVehicle") {
            $('.buy-container').fadeIn(150);
            N1Occasions.setupBuyContract(eventData, eventData.vehicleData)
        }
    });
});
...
$(document).on('click', '#sell-vehicle', function(){
    if ($('.vehicle-sell-price').val() != "") {
        if (!isNaN($('.vehicle-sell-price').val())) {
            $.post('http://N1-occasions/sellVehicle', JSON.stringify({
            ...
            $.post('http://N1-occasions/error', JSON.stringify({
...
$(document).on('click', '#buy-vehicle', function(){
    $.post('http://N1-occasions/buyVehicle');
    $('.buy-container').fadeOut(100);
    $.post('http://N1-occasions/close');
});

N1Occasions.setupSellContract = function(data) {
...
N1Occasions.setupBuyContract = function(data, vData) {
