...
RegisterNetEvent('N1-moneysafe:client:DepositMoney')
AddEventHandler('N1-moneysafe:client:DepositMoney', function(amount)
    if ClosestSafe ~= nil then
        if IsAuthorized then
            local ped = GetPlayerPed(-1)
            local pos = GetEntityCoords(ped)
            ...
            if distance < Config.MinimumSafeDistance then
                TriggerServerEvent('N1-moneysafe:server:DepositMoney', ClosestSafe, amount)
            end
        end
    end
end)

RegisterNetEvent('N1-moneysafe:client:WithdrawMoney')
AddEventHandler('N1-moneysafe:client:WithdrawMoney', function(amount)
    if ClosestSafe ~= nil then
        if IsAuthorized then
            local ped = GetPlayerPed(-1)
            local pos = GetEntityCoords(ped)
            ...
            if distance < Config.MinimumSafeDistance then
                TriggerServerEvent('N1-moneysafe:server:WithdrawMoney', ClosestSafe, amount)
            end
        end
    end
end)
...
RegisterNetEvent('N1-moneysafe:client:UpdateSafe')
AddEventHandler('N1-moneysafe:client:UpdateSafe', function(SafeData, k)
