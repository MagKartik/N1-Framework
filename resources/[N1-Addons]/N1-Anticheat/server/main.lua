...
N1Core.Functions.CreateCallback('N1-anticheat:server:GetPermissions', function(source, cb)
...
RegisterServerEvent('N1-anticheat:server:banPlayer')
AddEventHandler('N1-anticheat:server:banPlayer', function(reason)
    local src = source
    TriggerClientEvent('chatMessage', -1, "N1 Anti-Cheat", "error", GetPlayerName(src).." is verbannen voor: " ..reason )
    N1Core.Functions.ExecuteSql(false, "INSERT INTO `bans` (`name`, `steam`, `license`, `discord`,`ip`, `reason`, `expire`, `bannedby`) VALUES ('"..GetPlayerName(src).."', '"..GetPlayerIdentifiers(src)[1].."', '"..GetPlayerIdentifiers(src)[2].."', '"..GetPlayerIdentifiers(src)[3].."', '"..GetPlayerIdentifiers(src)[4].."', '"..reason.."', 2145913200, '"..GetPlayerName(src).."')")
    DropPlayer(src, "You have been flagged to much, for more information join our discord: https://discord.io/Lualeaks :)")
end)

-- Fake events --
function NonRegisteredEventCalled(CalledEvent, source)
    TriggerClientEvent("N1-anticheat:client:NonRegisteredEventCalled", source, "Cheating", CalledEvent)
end
...
N1Core.Functions.CreateCallback('N1-anticheat:server:HasWeaponInInventory', function(source, cb, WeaponInfo)
    local src = source
