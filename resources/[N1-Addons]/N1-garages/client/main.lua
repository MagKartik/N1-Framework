...
RegisterNetEvent('N1-garages:client:setHouseGarage')
AddEventHandler('N1-garages:client:setHouseGarage', function(house, hasKey)
    currentHouseGarage = house
    hasGarageKey = hasKey
end)

RegisterNetEvent('N1-garages:client:houseGarageConfig')
AddEventHandler('N1-garages:client:houseGarageConfig', function(garageConfig)
    HouseGarages = garageConfig
end)

RegisterNetEvent('N1-garages:client:addHouseGarage')
AddEventHandler('N1-garages:client:addHouseGarage', function(house, garageInfo)
    HouseGarages[house] = garageInfo
end)

-- function AddOutsideVehicle(plate, veh)
--     OutsideVehicles[plate] = veh
--     TriggerServerEvent('N1-garages:server:UpdateOutsideVehicles', OutsideVehicles)
-- end

RegisterNetEvent('N1-garages:client:takeOutDepot')
AddEventHandler('N1-garages:client:takeOutDepot', function(vehicle)
    if OutsideVehicles ~= nil and next(OutsideVehicles) ~= nil then
        if OutsideVehicles[vehicle.plate] ~= nil then
            ...
            if not VehExists then
                N1Core.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                    N1Core.Functions.TriggerCallback('N1-garage:server:GetVehicleProperties', function(properties)
                        N1Core.Functions.SetVehicleProperties(veh, properties)
                        ...
                        if vehicle.status ~= nil and next(vehicle.status) ~= nil then
                            TriggerServerEvent('N1-vehicletuning:server:LoadStatus', vehicle.status, vehicle.plate)
                        end
                        
                        if vehicle.drivingdistance ~= nil then
                            local amount = round(vehicle.drivingdistance / 1000, -2)
                            TriggerEvent('N1-hud:client:UpdateDrivingMeters', true, amount)
                            TriggerServerEvent('N1-vehicletuning:server:UpdateDrivingDistance', vehicle.drivingdistance, vehicle.plate)
                        end
                        ...
                        TriggerServerEvent('N1-garage:server:updateVehicleState', 0, vehicle.plate, vehicle.garage)
                        N1Core.Functions.Notify("Voertuig Uit: Motor: " .. enginePercent .. "% Body: " .. bodyPercent.. "% Fuel: "..currentFuel.. "%", "primary", 4500)
                        closeMenuFull()
                ...
                if Engine < 40.0 then
                    N1Core.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                        N1Core.Functions.TriggerCallback('N1-garage:server:GetVehicleProperties', function(properties)
                            ...
                            doCarDamage(veh, vehicle)
                            TriggerServerEvent('N1-garage:server:updateVehicleState', 0, vehicle.plate, vehicle.garage)
                            N1Core.Functions.Notify("Vehicle motor : off : " .. enginePercent .. "% Body: " .. bodyPercent.. "% Fuel: "..currentFuel.. "%", "primary", 4500)
                            closeMenuFull()
                            SetVehicleEngineOn(veh, true, true)
                        end, vehicle.plate)
                        TriggerEvent("vehiclekeys:client:SetOwner", vehicle.plate)
                    ...
                    N1Core.Functions.Notify('You cant duplicate the vehicle ..', 'error')
...
function AddTemporaryBlip(vehicle)  
    ...
    local VehicleData = N1Core.Shared.VehicleModels[GetEntityModel(vehicle)]
    ...
    N1Core.Functions.Notify("Your "..VehicleData["name"].." is temporary (1min) located on the map!", "success", 10000)

    SetTimeout(60 * 1000, function()
        N1Core.Functions.Notify('Your vehicle is not located on the map anymore!', 'error')
        RemoveBlip(TempBlip)
    end)
end
...
function MenuGarage()
    ...
    MenuTitle = "Garage"
    ClearMenu()
    Menu.addButton("My vehicle", "Vehicle kist", nil)
...
function HouseGarage(house)
    N1Core.Functions.TriggerCallback("N1-garage:server:GetHouseVehicles", function(result)
        ped = GetPlayerPed(-1);
        MenuTitle = "Vehicle depot :"
        ClearMenu()

        if result == nil then
            N1Core.Functions.Notify("You have no vehicle in your garage", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                local label = N1Core.Shared.Vehicles[v.vehicle]["name"]
                if N1Core.Shared.Vehicles[v.vehicle]["brand"] ~= nil then
                    label = N1Core.Shared.Vehicles[v.vehicle]["brand"].." "..N1Core.Shared.Vehicles[v.vehicle]["name"]
                end
...
function DepotLijst()
    N1Core.Functions.TriggerCallback("N1-garage:server:GetDepotVehicles", function(result)
        ped = GetPlayerPed(-1);
        MenuTitle = "Vehicle depot :"
        ClearMenu()

        if result == nil then
            N1Core.Functions.Notify("There is no vehicle in the depo", "error", 5000)
            closeMenuFull()
        else
...
function VoertuigLijst()
    N1Core.Functions.TriggerCallback("N1-garage:server:GetUserVehicles", function(result)
        ped = GetPlayerPed(-1);
        MenuTitle = "My Vehicles :"
        ClearMenu()

        if result == nil then
            N1Core.Functions.Notify("You have no vehicle in the depot", "error", 5000)
            closeMenuFull()
        else
...
function TakeOutVehicle(vehicle)
    ...
    elseif vehicle.state == "Uit" then
        N1Core.Functions.Notify("Is your vehicle in the depot??", "error", 2500)
    elseif vehicle.state == "Confiscated" then
        N1Core.Functions.Notify("This vehicle is confiscated by the police ", "error", 4000)
    end
end

function TakeOutDepotVehicle(vehicle)
    if vehicle.state == "Depot" then
        TriggerServerEvent("N1-garage:server:PayDepotPrice", vehicle)
    end
end
    ...
    if vehicle.state == "Garage" then
        N1Core.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
            N1Core.Functions.TriggerCallback('N1-garage:server:GetVehicleProperties', function(properties)
                ...
                doCarDamage(veh, vehicle)
                TriggerServerEvent('N1-garage:server:updateVehicleState', 0, vehicle.plate, vehicle.garage)
                N1Core.Functions.Notify("Vehicle engine off: " .. enginePercent .. "% Body: " .. bodyPercent.. "% Fuel: "..currentFuel.. "%", "primary", 4500)
                closeMenuFull()
                TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(veh))
                SetVehicleEngineOn(veh, true, true)
            end, vehicle.plate)
            ...
            if putDist <= 25 and IsPedInAnyVehicle(ped) then
                ...
                if putDist <= 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        N1Core.Functions.TriggerCallback('N1-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                local bodyDamage = math.ceil(GetVehicleBodyHealth(curVeh))
                                local engineDamage = math.ceil(GetVehicleEngineHealth(curVeh))
                                local totalFuel = exports['LegacyFuel']:GetFuel(curVeh)
        
                                TriggerServerEvent('N1-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, k)
                                TriggerServerEvent('N1-garage:server:updateVehicleState', 1, plate, k)
                                ...
                                N1Core.Functions.Notify("Vehicle parked in, "..Garages[k].label, "primary", 4500)
                            else
                                N1Core.Functions.Notify("No one is in possesion of no one..", "error", 3500)
                            end
                        end, plate)
                    end
                end
                    ...
                    if takeDist < 2.0 then
                        ...
                        elseif IsPedInAnyVehicle(ped) then
                            ...
                            if IsControlJustPressed(0, 38) then
                                local curVeh = GetVehiclePedIsIn(ped)
                                local plate = GetVehicleNumberPlateText(curVeh)
                                N1Core.Functions.TriggerCallback('N1-garage:server:checkVehicleHouseOwner', function(owned)
                                    if owned then
                                        local bodyDamage = round(GetVehicleBodyHealth(curVeh), 1)
                                        local engineDamage = round(GetVehicleEngineHealth(curVeh), 1)
                                        local totalFuel = exports['LegacyFuel']:GetFuel(curVeh)
                
                                        TriggerServerEvent('N1-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, currentHouseGarage)
                                        TriggerServerEvent('N1-garage:server:updateVehicleState', 1, plate, currentHouseGarage)
                                        N1Core.Functions.DeleteVehicle(curVeh)
                                        if plate ~= nil then
                                            OutsideVehicles[plate] = veh
                                            TriggerServerEvent('N1-garages:server:UpdateOutsideVehicles', OutsideVehicles)
                                        end
                                        N1Core.Functions.Notify("Vehicle is parked in, "..HouseGarages[currentHouseGarage].label, "primary", 4500)
                                    else
                                        N1Core.Functions.Notify("No one is in possesion of this vehicle ...", "error", 3500)
                                    end
