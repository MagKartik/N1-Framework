...
RegisterServerEvent('N1-garages:server:RemoveVehicle')
AddEventHandler('N1-garages:server:RemoveVehicle', function(CitizenId, Plate)
    if OutsideVehicles[CitizenId] ~= nil then
        OutsideVehicles[CitizenId][Plate] = nil
    end
end)

RegisterServerEvent('N1-garages:server:UpdateOutsideVehicles')
AddEventHandler('N1-garages:server:UpdateOutsideVehicles', function(Vehicles)
    local src = source
...
N1Core.Functions.CreateCallback("N1-garage:server:GetOutsideVehicles", function(source, cb)
...
N1Core.Functions.CreateCallback("N1-garage:server:GetUserVehicles", function(source, cb, garage)
    local src = source
...
N1Core.Functions.CreateCallback("N1-garage:server:GetVehicleProperties", function(source, cb, plate)
    local src = source
    local properties = {}
    N1Core.Functions.ExecuteSql(false, "SELECT `mods` FROM `player_vehicles` WHERE `plate` = '"..plate.."'", function(result)
        if result[1] ~= nil then
...
N1Core.Functions.CreateCallback("N1-garage:server:GetDepotVehicles", function(source, cb)
    local src = source
    local pData = N1Core.Functions.GetPlayer(src)
...
N1Core.Functions.CreateCallback("N1-garage:server:GetHouseVehicles", function(source, cb, house)
    local src = source
    local pData = N1Core.Functions.GetPlayer(src)
...
N1Core.Functions.CreateCallback("N1-garage:server:checkVehicleHouseOwner", function(source, cb, plate, house)
    local src = source
    local pData = N1Core.Functions.GetPlayer(src)
    exports['ghmattimysql']:execute('SELECT * FROM player_vehicles WHERE plate = @plate', {['@plate'] = plate}, function(result)
        if result[1] ~= nil then
            local hasHouseKey = exports['N1-houses']:hasKey(result[1].steam, result[1].citizenid, house)
            if hasHouseKey then
                cb(true)
            else
                cb(false)
...
RegisterServerEvent('N1-garage:server:PayDepotPrice')
AddEventHandler('N1-garage:server:PayDepotPrice', function(vehicle, garage)
    local src = source
    ...
    exports['ghmattimysql']:execute('SELECT * FROM player_vehicles WHERE plate = @plate', {['@plate'] = vehicle.plate}, function(result)
        if result[1] ~= nil then
            if Player.Functions.RemoveMoney("cash", result[1].depotprice, "paid-depot") then
                TriggerClientEvent("N1-garages:client:takeOutDepot", src, vehicle, garage)
...
RegisterServerEvent('N1-garage:server:updateVehicleState')
AddEventHandler('N1-garage:server:updateVehicleState', function(state, plate, garage)
    local src = source
    local pData = N1Core.Functions.GetPlayer(src)
...
RegisterServerEvent('N1-garage:server:updateVehicleStatus')
AddEventHandler('N1-garage:server:updateVehicleStatus', function(fuel, engine, body, plate, garage)
    local src = source
    local pData = N1Core.Functions.GetPlayer(src)
