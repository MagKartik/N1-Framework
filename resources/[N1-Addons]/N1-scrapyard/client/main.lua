...
AddEventHandler("N1Core:Client:OnPlayerLoaded", function()
    TriggerServerEvent("N1-scrapyard:server:LoadVehicleList")
end)
							...
							if IsControlJustReleased(0, Keys["E"]) then
								if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										N1Core.Functions.Notify("This vehicle can\'t be scrapped..", "error")
									end
								else
									N1Core.Functions.Notify("You're not the driver..", "error")
								end
							end
						end
					end
...
RegisterNetEvent('N1-scapyard:client:setNewVehicles')
AddEventHandler('N1-scapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = N1Core.Shared.Vehicles[v]
		...
		SetTimeout(math.random(15000, 20000), function()
			...
			TriggerServerEvent('N1-phone:server:sendNewMail', {
				...
				message = "You can only scrap a number of vehicles.<br />You can keep everything you scrap for yourself as long as you don't bother me.<br /><br /><strong>Vehicle list:</strong><br />".. vehicleList,
				button = {}
			})
		end)
	else
		N1Core.Functions.Notify("You can\'t scrap any cars now..", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	N1Core.Functions.Progressbar("scrap_vehicle", "Scrapping vehicle..", scrapTime, false, true, {
		...
		TriggerServerEvent("N1-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))
