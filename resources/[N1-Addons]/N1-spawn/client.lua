...
RegisterNetEvent('N1-spawn:client:openUI')
AddEventHandler('N1-spawn:client:openUI', function(value)
...
RegisterNUICallback('setCam', function(data)
    ...
    elseif type == "normal" then
        local campos = N1.Spawns[location].coords
...
RegisterNUICallback('chooseAppa', function(data)
    ...
    TriggerServerEvent('N1Core:Server:OnPlayerLoaded')
    TriggerEvent('N1Core:Client:OnPlayerLoaded')
...
RegisterNUICallback('spawnplayer', function(data)
    ...
    if type == "current" then
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('N1-houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('N1-apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('N1Core:Server:OnPlayerLoaded')
        TriggerEvent('N1Core:Client:OnPlayerLoaded')
        FreezeEntityPosition(ped, false)
    ...
    elseif type == "house" then
        ...
        DoScreenFadeOut(500)
        Citizen.Wait(2000)
        TriggerEvent('N1-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('N1Core:Server:OnPlayerLoaded')
        TriggerEvent('N1Core:Client:OnPlayerLoaded')
        TriggerServerEvent('N1-houses:server:SetInsideMeta', 0, false)
        TriggerServerEvent('N1-apartments:server:SetInsideMeta', 0, 0, false)
        FreezeEntityPosition(ped, false)
        RenderScriptCams(false, true, 500, true, true)
        SetCamActive(cam, false)
        DestroyCam(cam, true)
    ...
    elseif type == "normal" then
        print('normal')
        local pos = N1.Spawns[location].coords
        SetDisplay(false)
        DoScreenFadeOut(500)
        Citizen.Wait(2000)
...
RegisterNetEvent('N1-houses:client:setHouseConfig')
AddEventHandler('N1-houses:client:setHouseConfig', function(houseConfig)
    Config.Houses = houseConfig
end)

RegisterNetEvent('N1-spawn:client:setupSpawns')
AddEventHandler('N1-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        N1Core.Functions.TriggerCallback('N1-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
