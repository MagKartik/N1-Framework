...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(GetPlayerPed(-1)) then
            ...
            if IsControlJustReleased(0, Keys["Y"]) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then 
                if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    cruiseSpeed = Speed
                    if cruiseOn then
                        N1Core.Functions.Notify("Limiter shut off!")
                    else
                        N1Core.Functions.Notify("Limiter activated on "..tostring(math.floor(cruiseSpeed * 3.6)).."km/u")
                    end
