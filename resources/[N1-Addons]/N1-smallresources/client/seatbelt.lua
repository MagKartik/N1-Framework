    ...
    while true do
        ...
        if IsControlJustReleased(0, Keys["B"]) then 
            if not harnessOn then
                ...
                N1Core.Functions.Progressbar("harness_equip", "Take off race harness..", 5000, false, true, {
...
function ToggleHarness(toggle)
    ...
    TriggerEvent('N1-hud:client:ToggleHarness', toggle)
end
...
AddEventHandler('seatbelt:client:UseHarness', function(ItemData)
    ...
    if inveh and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
        if not harnessOn then
            N1Core.Functions.Progressbar("harness_equip", "Race harness on..", 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        N1Core.Functions.Notify('You\'re not in a car.', 'error', 3500)
    end
end)
