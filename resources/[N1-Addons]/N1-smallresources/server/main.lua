...
N1Core.Functions.CreateCallback('nos:GetNosLoadedVehs', function(source, cb)
    cb(VehicleNitrous)
end)

N1Core.Commands.Add("id", "Wat is mijn id?", {}, false, function(source, args)
    TriggerClientEvent('chatMessage', source, "SYSTEM", "warning", "ID: "..source)
end)

N1Core.Functions.CreateUseableItem("harness", function(source, item)
...
AddEventHandler('equip:harness', function(item)
    ...
    if Player.PlayerData.items[item.slot].info.uses - 1 == 0 then
        TriggerClientEvent("inventory:client:ItemBox", source, N1Core.Shared.Items['harness'], "remove")
