...
AddEventHandler("N1Core:Client:OnPlayerLoaded", function()
    ...
    N1Core.Functions.TriggerCallback('N1-bankrobbery:server:GetConfig', function(config)
        Config = config
    end)
    onDuty = true
    ResetBankDoors()
...
Citizen.CreateThread(function()
    Wait(500)
    if N1Core.Functions.GetPlayerData() ~= nil then
        PlayerJob = N1Core.Functions.GetPlayerData().job
        onDuty = true
    end
end)
...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = N1Core.Shared.Items["electronickit"]["name"], image = N1Core.Shared.Items["electronickit"]["image"]},
        [2] = {name = N1Core.Shared.Items["trojan_usb"]["name"], image = N1Core.Shared.Items["trojan_usb"]["image"]},
    }
    while true do
        local ped = GetPlayerPed(-1)
        local pos = GetEntityCoords(ped)
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, Keys["E"]) then
                                            if CurrentCops >= Config.MinimumFleecaPolice then
                                                ...
                                                N1Core.Functions.Notify("Not enough police.. (4 needed)", "error")
                                            end
                                        end
                                    end
                                end
...
AddEventHandler('electronickit:UseElectronickit', function()
    ...
    if closestBank ~= nil then
        N1Core.Functions.TriggerCallback('N1-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if closestBank ~= nil then
                    local dist = GetDistanceBetweenCoords(pos, Config.SmallBanks[closestBank]["coords"]["x"], Config.SmallBanks[closestBank]["coords"]["y"], Config.SmallBanks[closestBank]["coords"]["z"])
                    if dist < 1.5 then
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            if not Config.SmallBanks[closestBank]["isOpened"] then 
                                N1Core.Functions.TriggerCallback('N1Core:HasItem', function(result)
                                    if result then 
                                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                        N1Core.Functions.Progressbar("hack_gate", "Connecting electronic kit..", math.random(5000, 10000), false, true, {
                                            ...
                                            if not copsCalled then
                                                ...
                                                if Config.SmallBanks[closestBank]["alarm"] then
                                                    TriggerServerEvent("N1-bankrobbery:server:callCops", "small", closestBank, streetLabel, pos)
                                                    copsCalled = true
                                                end
                                            end
                                        end, function() -- Cancel
                                        ...
                                        N1Core.Functions.Notify("You are missing something..", "error")
                                    end
                                end, "trojan_usb")
                            else
                                N1Core.Functions.Notify("It looks like the bank is already opened..", "error")
                            end
                        else
                            N1Core.Functions.Notify("Not enough police.. (4 needed)", "error")
                        end
                ...
                N1Core.Functions.Notify("The security lock is active, the door cannot be opened at the moment..", "error", 5500)
            end
        end)
    end
end)

RegisterNetEvent('N1-bankrobbery:client:setBankState')
AddEventHandler('N1-bankrobbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('N1-bankrobbery:client:enableAllBankSecurity')
AddEventHandler('N1-bankrobbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('N1-bankrobbery:client:disableAllBankSecurity')
AddEventHandler('N1-bankrobbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('N1-bankrobbery:client:BankSecurity')
AddEventHandler('N1-bankrobbery:client:BankSecurity', function(key, status)
...
function openLocker(bankId, lockerId)
    ...
    TriggerServerEvent('N1-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "paleto" then
        N1Core.Functions.TriggerCallback('N1-radio:server:GetItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                N1Core.Functions.Progressbar("open_locker_drill", "Cracking the save..", math.random(40000, 60000), false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    TriggerServerEvent('N1-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('N1-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('N1-bankrobbery:server:recieveItem', 'paleto')
                    N1Core.Functions.Notify("Succes!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(GetPlayerPed(-1), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('N1-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                ...
                Citizen.CreateThread(function()
                    while IsDrilling do
                        TriggerServerEvent('N1-hud:Server:GainStress', math.random(4, 8))
                        Citizen.Wait(10000)
                    end
                end)
            else
                N1Core.Functions.Notify("It looks like the safe lock is too strong..", "error")
                TriggerServerEvent('N1-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        N1Core.Functions.TriggerCallback('N1-radio:server:GetItem', function(hasItem)
            if hasItem then
                ...
                N1Core.Functions.Progressbar("open_locker_drill", "Cracking the save..", math.random(40000, 60000), false, true, {
                    ...
                    TriggerServerEvent('N1-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('N1-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('N1-bankrobbery:server:recieveItem', 'pacific')
                    N1Core.Functions.Notify("Succes!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(GetPlayerPed(-1), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
        ...
        IsDrilling = true
        N1Core.Functions.Progressbar("open_locker", "Cracking the save..", math.random(24000, 48000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('N1-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
            TriggerServerEvent('N1-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            TriggerServerEvent('N1-bankrobbery:server:recieveItem', 'small')
            N1Core.Functions.Notify("Succes!", "success")
            IsDrilling = false
        end, function() -- Cancel
            StopAnimTask(GetPlayerPed(-1), "anim@gangops@facility@servers@", "hotwire", 1.0)
...
RegisterNetEvent('N1-bankrobbery:client:setLockerState')
AddEventHandler('N1-bankrobbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterNetEvent('N1-bankrobbery:client:ResetFleecaLockers')
AddEventHandler('N1-bankrobbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent('N1-bankrobbery:client:robberyCall')
AddEventHandler('N1-bankrobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then 
        ...
        if type == "small" then
            ...
            TriggerEvent('N1-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                ...
                callSign = N1Core.Functions.GetPlayerData().metadata["callsign"],
            })
...
function OnHackDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('N1-bankrobbery:server:setBankState', closestBank, true)
    else
		TriggerEvent('mhacking:hide')
	end
end
