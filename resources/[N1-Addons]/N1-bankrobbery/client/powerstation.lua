        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    N1Core.Functions.Notify("It seems that the fuses have burned out..", "error")
                end
            else
                N1Core.Functions.Notify("Not enough police (2 needed)", "error")
            end
        end
...
RegisterNetEvent('N1-bankrobbery:client:SetStationStatus')
AddEventHandler('N1-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(GetPlayerPed(-1))
    while time > 0 do 
        N1Core.Functions.Notify("Burning in " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    local randTime = math.random(10000, 15000)
    ...
    if currentStation ~= 0 then
        N1Core.Functions.Notify("The fuses are broken", "success")
        TriggerServerEvent("N1-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        N1Core.Functions.Notify("The door is burned open", "success")
        TriggerServerEvent('N1-doorlock:server:updateState', currentGate, false)
        currentGate = 0
    end
end)
