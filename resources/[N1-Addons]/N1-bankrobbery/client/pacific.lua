...
Citizen.CreateThread(function()
    ...
    local requiredItems = {
        [1] = {name = N1Core.Shared.Items["security_card_02"]["name"], image = N1Core.Shared.Items["security_card_02"]["image"]},
    }
    while true do
                                ...
                                if lockerDist < 0.5 then
                                    ...
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        if CurrentCops >= Config.MinimumPacificPolice then
                                            ...
                                            N1Core.Functions.Notify("Not enough police.. (6 needed)", "error")
                                        end
                                    end
                                end
                            end
                                    ...
                                    N1Core.Functions.Progressbar("hack_gate", "Connecting electronic kit..", math.random(5000, 10000), false, true, {
                                        ...
                                        if not copsCalled then
                                            ...
                                            if Config.BigBanks["pacific"]["alarm"] then
                                                TriggerServerEvent("N1-bankrobbery:server:callCops", "pacific", 0, streetLabel, pos)
                                                copsCalled = true
                                            end
                                        end
                                    end, function() -- Cancel
...
RegisterNetEvent('N1-bankrobbery:UseBankcardB')
AddEventHandler('N1-bankrobbery:UseBankcardB', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
        ...
        N1Core.Functions.TriggerCallback('N1-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems2, false)
                        N1Core.Functions.Progressbar("security_pass", "Pas aan het valideren..", math.random(5000, 10000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            StopAnimTask(GetPlayerPed(-1), "anim@gangops@facility@servers@", "hotwire", 1.0)
                            TriggerServerEvent('N1-doorlock:server:updateState', 76, false)
                            TriggerServerEvent("N1Core:Server:RemoveItem", "security_card_02", 1)
                            if not copsCalled then
                                local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, pos.x, pos.y, pos.z, Citizen.PointerValueInt(), Citizen.PointerValueInt())
                                local street1 = GetStreetNameFromHashKey(s1)
                                local street2 = GetStreetNameFromHashKey(s2)
...
function OnHackPacificDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('N1-bankrobbery:server:setBankState', "pacific", true)
    else
		TriggerEvent('mhacking:hide')
	end
end
