...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = N1Core.Shared.Items["security_card_01"]["name"], image = N1Core.Shared.Items["security_card_01"]["image"]},
    }
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, Keys["E"]) then
                                            if CurrentCops >= Config.MinimumPaletoPolice then
                                                ...
                                                N1Core.Functions.Notify("Not enough police.. (5 needed)", "error")
                                            end
                                        end
                                    end
                                end
...
RegisterNetEvent('N1-bankrobbery:UseBankcardA')
AddEventHandler('N1-bankrobbery:UseBankcardA', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local dist = GetDistanceBetweenCoords(pos, Config.BigBanks["paleto"]["coords"]["x"], Config.BigBanks["paleto"]["coords"]["y"],Config.BigBanks["paleto"]["coords"]["z"])
        ...
        N1Core.Functions.TriggerCallback('N1-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        N1Core.Functions.Progressbar("security_pass", "Validitating card..", math.random(5000, 10000), false, true, {
                            ...
                            TriggerServerEvent('N1-bankrobbery:server:setBankState', "paleto", true)
                            TriggerServerEvent("N1Core:Server:RemoveItem", "security_card_01", 1)
                            TriggerServerEvent('N1-doorlock:server:updateState', 85, false)
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("N1-bankrobbery:server:callCops", "paleto", 0, streetLabel, pos)
                                    copsCalled = true
                                end
                            end
                        end, function() -- Cancel
