...
function init($param) {
    ...
    if($param!=1)
        $("#container > p").append("<br><h4>at the right moment</h4>");
    else
        $("#container > p").append("<br><h4>at the right moment</h4>");
    $('.time').addClass('done');
}
...
$(document).ready(function() {
    ...
    window.addEventListener('click',function() {
        ...
        if (distance < HitDistance) {
            ...
            MainDigits--;
            $.post("http://N1-bankrobbery/thermiteclick");
...
Thermite.Failed = function() {
    if (active) $.post("http://N1-bankrobbery/thermitefailed");
    active = false;
    Thermite.Close();
}

Thermite.Success = function() {
    if (active) $.post("http://N1-bankrobbery/thermitesuccess");
    active = false;
    Thermite.Close();
}
...
Thermite.Close = function() {
    ...
    $.post('http://N1-bankrobbery/closethermite');
}