...
Citizen.CreateThread(function()
    while true do
        ...
        if blackoutActive then
            TriggerEvent("N1-weathersync:server:toggleBlackout")
            TriggerClientEvent("police:client:EnableAllCameras", -1)
            TriggerClientEvent("N1-bankrobbery:client:enableAllBankSecurity", -1)
...
RegisterServerEvent('N1-bankrobbery:server:setBankState')
AddEventHandler('N1-bankrobbery:server:setBankState', function(bankId, state)
    if bankId == "paleto" then
        if not robberyBusy then
            Config.BigBanks["paleto"]["isOpened"] = state
            TriggerClientEvent('N1-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('N1-scoreboard:server:SetActivityBusy', "bankrobbery", true)
            TriggerEvent('N1-bankrobbery:server:setTimeout')
        end
    elseif bankId == "pacific" then
        if not robberyBusy then
            Config.BigBanks["pacific"]["isOpened"] = state
            TriggerClientEvent('N1-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('N1-scoreboard:server:SetActivityBusy', "pacific", true)
            TriggerEvent('N1-bankrobbery:server:setTimeout')
        end
    else
        if not robberyBusy then
            ...
            TriggerClientEvent('N1-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('N1-banking:server:SetBankClosed', bankId, true)
            TriggerEvent('N1-scoreboard:server:SetActivityBusy', "bankrobbery", true)
            TriggerEvent('N1-bankrobbery:server:SetSmallbankTimeout', bankId)
        end
    end
    robberyBusy = true
end)

RegisterServerEvent('N1-bankrobbery:server:setLockerState')
AddEventHandler('N1-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
    ...
    TriggerClientEvent('N1-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterServerEvent('N1-bankrobbery:server:recieveItem')
AddEventHandler('N1-bankrobbery:server:recieveItem', function(type)
    local src = source
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == "item" then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items[item.item], "add")
                elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = math.random(4000, 6000)
                    }
                    ply.Functions.AddItem('markedbills', 1, false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items['markedbills'], "add")
                end
            else
                ply.Functions.AddItem('security_card_01', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items['security_card_01'], "add")
            end
        else
            ply.Functions.AddItem('weapon_stungun', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items['weapon_stungun'], "add")
        end
    elseif type == "paleto" then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items['security_card_02'], "add")
            end
        else
            ply.Functions.AddItem('weapon_vintagepistol', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items['weapon_vintagepistol'], "add")
        end
    elseif type == "pacific" then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items['cryptostick'], "add")
            end
        else
            ...
            if chance == odd then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items['weapon_microsmg'], "add")
            else
                ply.Functions.AddItem('weapon_minismg', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items['weapon_minismg'], "add")
            end
        end
    end
end)

N1Core.Functions.CreateCallback('N1-bankrobbery:server:isRobberyActive', function(source, cb)
    cb(robberyBusy)
end)

N1Core.Functions.CreateCallback('N1-bankrobbery:server:GetConfig', function(source, cb)
    cb(Config)
end)

RegisterServerEvent('N1-bankrobbery:server:setTimeout')
AddEventHandler('N1-bankrobbery:server:setTimeout', function()
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
                TriggerEvent('N1-scoreboard:server:SetActivityBusy', "bankrobbery", false)
                TriggerEvent('N1-scoreboard:server:SetActivityBusy', "pacific", false)
                ...
                TriggerClientEvent('N1-bankrobbery:client:ClearTimeoutDoors', -1)
...
RegisterServerEvent('N1-bankrobbery:server:SetSmallbankTimeout')
AddEventHandler('N1-bankrobbery:server:SetSmallbankTimeout', function(BankId)
    if not robberyBusy then
        SetTimeout(30 * (60 * 1000), function()
            ...
            timeOut = false
            robberyBusy = false
            TriggerClientEvent('N1-bankrobbery:client:ResetFleecaLockers', -1, BankId)
            TriggerEvent('N1-banking:server:SetBankClosed', BankId, false)
        end)
    end
end)

RegisterServerEvent('N1-bankrobbery:server:callCops')
AddEventHandler('N1-bankrobbery:server:callCops', function(type, bank, streetLabel, coords)
    local cameraId = 4
    ...
    TriggerClientEvent("N1-bankrobbery:client:robberyCall", -1, type, bank, streetLabel, coords)
    TriggerClientEvent("N1-phone:client:addPoliceAlert", -1, alertData)
end)

RegisterServerEvent('N1-bankrobbery:server:SetStationStatus')
AddEventHandler('N1-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent("N1-bankrobbery:client:SetStationStatus", -1, key, isHit)
    if AllStationsHit() then
        TriggerEvent("N1-weathersync:server:toggleBlackout")
        TriggerClientEvent("police:client:DisableAllCameras", -1)
        TriggerClientEvent("N1-bankrobbery:client:disableAllBankSecurity", -1)
        blackoutActive = true
    else
        CheckStationHits()
    end
...
function CheckStationHits()
    ...
    if Config.PowerStations[11].hit and Config.PowerStations[1].hit and Config.PowerStations[2].hit then
        ...
        TriggerClientEvent("N1-bankrobbery:client:BankSecurity", 1, false)
        TriggerClientEvent("police:client:SetCamera", -1, 22, false)
        TriggerClientEvent("N1-bankrobbery:client:BankSecurity", 2, false)
    end
    if Config.PowerStations[8].hit and Config.PowerStations[4].hit and Config.PowerStations[5].hit and Config.PowerStations[6].hit then
        ...
        TriggerClientEvent("N1-bankrobbery:client:BankSecurity", 3, false)
    end
    if Config.PowerStations[12].hit and Config.PowerStations[13].hit then
        TriggerClientEvent("police:client:SetCamera", -1, 24, false)
        TriggerClientEvent("N1-bankrobbery:client:BankSecurity", 4, false)
        TriggerClientEvent("police:client:SetCamera", -1, 25, false)
        TriggerClientEvent("N1-bankrobbery:client:BankSecurity", 5, false)
    end
end
...
N1Core.Functions.CreateUseableItem("thermite", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('lighter') ~= nil then
        TriggerClientEvent("thermite:UseThermite", source)
    else
        TriggerClientEvent('N1Core:Notify', source, "You are missing something to light the thermite..", "error")
    end
end)

N1Core.Functions.CreateUseableItem("security_card_01", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_01') ~= nil then
        TriggerClientEvent("N1-bankrobbery:UseBankcardA", source)
    end
end)

N1Core.Functions.CreateUseableItem("security_card_02", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_02') ~= nil then
        TriggerClientEvent("N1-bankrobbery:UseBankcardB", source)
    end
end)