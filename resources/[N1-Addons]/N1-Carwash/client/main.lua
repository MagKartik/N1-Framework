                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('N1-carwash:server:washCar')
                                end
                            else
...
RegisterNetEvent('N1-carwash:client:washCar')
AddEventHandler('N1-carwash:client:washCar', function()
    local PlayerPed = GetPlayerPed(-1)
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    N1Core.Functions.Progressbar("search_cabin", "Vehicle is being washed..", math.random(4000, 8000), false, true, {
        ...
        N1Core.Functions.Notify("Washing canceled..", "error")
        washingVehicle = false
    end)
end)
