...
RentVehicle = function(selectedVehicle)
    ...
    if IsPedInAnyVehicle(ped) then
        N1Core.Functions.Notify('this can only when not in a vehicle..', 'error')
        return
    end

    local vehiclePlate = "N1R"..math.random(1, 100)
    TriggerServerEvent('N1-vehiclerental:server:SetVehicleRented', vehiclePlate, true, Config.RentalVehicles[selectedVehicle])

    RentedVehicleData = selectedVehicle
end

RegisterNetEvent('N1-vehiclerental:server:SpawnRentedVehicle')
AddEventHandler('N1-vehiclerental:server:SpawnRentedVehicle', function(vehiclePlate, vehicleData)
    local ped = GetPlayerPed(-1)
    local coords = {
...
ReturnVehicle = function()
    ...
    N1Core.Functions.Notify('You do not have a deposit open..', 'error')
end
...
AcceptReturn = function()
    ...
    if noSpace(VehiclePlate) ~= noSpace(RentedVehiclePlate) then
        N1Core.Functions.Notify('This is not a rental vehicle..', 'error')
    else
        TriggerServerEvent('N1-vehiclerental:server:SetVehicleRented', RentedVehiclePlate, false, Config.RentalVehicles[RentedVehicleData])
        N1Core.Functions.DeleteVehicle(CurrentVehicle)
