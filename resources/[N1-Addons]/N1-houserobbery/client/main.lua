...
AddEventHandler('N1Core:Client:OnPlayerLoaded', function()
    N1Core.Functions.TriggerCallback('N1-houserobbery:server:GetHouseConfig', function(HouseConfig)
...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    requiredItems = {
        [1] = {name = N1Core.Shared.Items["lockpick"]["name"], image = N1Core.Shared.Items["lockpick"]["image"]},
        [2] = {name = N1Core.Shared.Items["screwdriverset"]["name"], image = N1Core.Shared.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
...
function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['N1-interior']:CreateTier1HouseFurnished(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('N1-houserobbery:client:ResetHouseState')
AddEventHandler('N1-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('N1-houserobbery:client:enterHouse')
AddEventHandler('N1-houserobbery:client:enterHouse', function(house)
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        PoliceCall()
                        TriggerEvent('N1-lockpick:client:openLockpick', lockpickFinish)
                        if math.random(1, 100) <= 85 and not IsWearingHandshoes() then
                            local pos = GetEntityCoords(GetPlayerPed(-1))
                            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
                        end
                    else
                        N1Core.Functions.Notify('Door is already open..', 'error', 3500)
                    end
                else
                    N1Core.Functions.Notify('Not enough cops..', 'error', 3500)
                end
            end
        else
                    ...
                    if result then
                        if CurrentCops >= Config.MinimumHouseRobberyPolice then
                            if not Config.Houses[closestHouse]["opened"] then
                                ...
                                N1Core.Functions.Notify('Door is open already..', 'error', 3500)
                            end
                        else
                            N1Core.Functions.Notify('Not enough cops..', 'error', 3500)
                        end
                    else
                        N1Core.Functions.Notify('It looks like you\'re missing something...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
...
function PoliceCall()
    ...
    if math.random(1, 100) <= chance then
        ...
        if closestPed ~= nil then
            ...
            local gender = "Man"
            if N1Core.Functions.GetPlayerData().charinfo.gender == 1 then
                gender = "Woman"
            end
...
function lockpickFinish(success)
    if success then
        TriggerServerEvent('N1-houserobbery:server:enterHouse', closestHouse)
        N1Core.Functions.Notify('You did it!', 'success', 2500)
    else
        if usingAdvanced then
            local itemInfo = N1Core.Shared.Items["advancedlockpick"]
        ...
        N1Core.Functions.Notify('It didn\'t work..', 'error', 2500)
    end
end

RegisterNetEvent('N1-houserobbery:client:setHouseState')
AddEventHandler('N1-houserobbery:client:setHouseState', function(house, state)
...
function searchCabin(cabin)
    ...
    local Skillbar = exports['N1-skillbar']:GetSkillbarObject()
    if math.random(1, 100) <= 85 and not IsWearingHandshoes() then
        local pos = GetEntityCoords(GetPlayerPed(-1))
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
    end
    LockpickDoorAnim(lockpickTime)
    TriggerServerEvent('N1-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            openingDoor = false
            ClearPedTasks(GetPlayerPed(-1))
            TriggerServerEvent('N1-houserobbery:server:searchCabin', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('N1-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('N1-houserobbery:client:setCabinState')
AddEventHandler('N1-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('N1-houserobbery:client:SetBusyState')
AddEventHandler('N1-houserobbery:client:SetBusyState', function(cabin, house, bool)
