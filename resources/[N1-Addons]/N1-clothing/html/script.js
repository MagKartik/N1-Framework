...
N1Clothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('http://N1-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('http://N1-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('http://N1-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
                    $("#current-model").html("<p>"+model+"</p>")
                });
                canChange = true;
                N1Clothing.ResetValues()
                ...
                if (buttonType == "item") {
                    N1Clothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('http://N1-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('http://N1-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('http://N1-clothing/rotateRight');
            break;
        case 65: // A
            $.post('http://N1-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
N1Clothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                N1Clothing.Open(event.data);
                break;
            case "close":
                N1Clothing.Close();
                break;
            case "updateMax":
                N1Clothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                N1Clothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                N1Clothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                N1Clothing.ResetValues();
                break;
...
N1Clothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    N1Clothing.Close();
    $.post('http://N1-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    N1Clothing.Close();
    $.post('http://N1-clothing/resetOutfit');
});

N1Clothing.SetCurrentValues = function(clothingValues) {
...
N1Clothing.Open = function(data) {
    ...
    N1Clothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    N1Clothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('http://N1-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('http://N1-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
N1Clothing.Close = function() {
    $.post('http://N1-clothing/close');
...
N1Clothing.SetMaxValues = function(maxValues) {
...
N1Clothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('http://N1-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('http://N1-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
