...
AddEventHandler('N1Core:Client:OnPlayerLoaded', function()
    TriggerServerEvent("N1-clothes:loadPlayerSkin")
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    gender = "male"
                                    if N1Core.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    N1Core.Functions.TriggerCallback('N1-clothing:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNetEvent('N1-clothing:client:openOutfitMenu')
AddEventHandler('N1-clothing:client:openOutfitMenu', function()
    N1Core.Functions.TriggerCallback('N1-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('N1-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('N1-clothing:client:openMenu')
AddEventHandler('N1-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('N1-clothes:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('N1-clothing:client:reloadOutfits')
AddEventHandler('N1-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('N1-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
function SaveSkin()
	...
	TriggerServerEvent("N1-clothing:saveSkin", model, clothing)
end

RegisterNetEvent('N1-clothes:client:CreateFirstCharacter')
AddEventHandler('N1-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("N1-clothes:loadSkin")
AddEventHandler("N1-clothes:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('N1-clothing:client:loadPlayerClothing', data, GetPlayerPed(-1))
    end)
end)

RegisterNetEvent('N1-clothing:client:loadPlayerClothing')
AddEventHandler('N1-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('N1-clothing:client:loadOutfit')
AddEventHandler('N1-clothing:client:loadOutfit', function(oData)
    local ped = GetPlayerPed(-1)
    ...
    if data["accessory"] ~= nil then
        if N1Core.Functions.GetPlayerData().metadata["tracker"] then
...
RegisterNetEvent("N1-clothing:client:adjustfacewear")
AddEventHandler("N1-clothing:client:adjustfacewear",function(type)
    if N1Core.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
