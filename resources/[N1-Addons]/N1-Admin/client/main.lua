...
N1Admin.Functions.DrawText3D = function(x, y, z, text, lines)
...
AddEventHandler('N1Core:Client:OnPlayerLoaded', function()
    TriggerServerEvent("N1-admin:server:loadPermissions")
end)
...
RegisterNetEvent('N1-admin:client:openMenu')
AddEventHandler('N1-admin:client:openMenu', function(group, dealers)
...
Citizen.CreateThread(function()
    ...
    local currentPermIndex = 1w
    local selectedPermIndex = 1

    WarMenu.CreateMenu('admin', 'N1-FW | Admin')
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('selfOptions') then
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                ...
                TriggerServerEvent("N1-admin:server:togglePlayerNoclip", targetId)
            end
            if WarMenu.Button('Revive') then
                local target = PlayerId()
                local targetId = GetPlayerServerId(target)
                TriggerServerEvent('N1-admin:server:revivePlayer', targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('serverMan') then
            ...
            if WarMenu.ComboBox('Server time', times, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent("N1-weathersync:server:setTime", time.hour, time.minute)
            end
        ...
        elseif WarMenu.IsMenuOpened('playerOptions') then
            if WarMenu.MenuButton('Kill', currentPlayer) then
                TriggerServerEvent("N1-admin:server:killPlayer", GetPlayerServerId(currentPlayer))
            end
            if WarMenu.MenuButton('Revive', currentPlayer) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent('N1-admin:server:revivePlayer', target)
            end
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("N1-admin:server:togglePlayerNoclip", target)
            end
            if WarMenu.CheckBox("Freeze", isFreeze, function(checked) isFreeze = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("N1-admin:server:Freeze", target, isFreeze)
            end
            ...
            if WarMenu.MenuButton("Give Clothing Menu", currentPlayer) then
                ...
                TriggerServerEvent('N1-admin:server:OpenSkinMenu', targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('teleportOptions') then
            ...
            if WarMenu.MenuButton('Bring', currentPlayer) then
                ...
                TriggerServerEvent('N1-admin:server:bringTp', GetPlayerServerId(currentPlayer), plyCoords)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('permissionOptions') then
            if WarMenu.ComboBox('Permission Group', perms, currentPermIndex, selectedPermIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent('N1-admin:server:setPermissions', target, group)

                N1Core.Functions.Notify('You changed '..GetPlayerName(currentPlayer)..'\'s group to '..group.label)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('adminOptions') then
            if WarMenu.ComboBox('Ban length', bans, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                if reason ~= nil and reason ~= "" and time ~= 0 then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("N1-admin:server:banPlayer", target, time, reason)
                end
            end
            if WarMenu.MenuButton('Kick', currentPlayer) then
                ...
                if reason ~= nil and reason ~= "" then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("N1-admin:server:kickPlayer", target, reason)
                end
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('weatherOptions') then
            for k, v in pairs(AvailableWeatherTypes) do
                if WarMenu.MenuButton(AvailableWeatherTypes[k].label, 'weatherOptions') then
                    TriggerServerEvent('N1-weathersync:server:setWeather', AvailableWeatherTypes[k].weather)
                    N1Core.Functions.Notify('Weather changed to: '..AvailableWeatherTypes[k].label)
                end
            end
        ...
        elseif WarMenu.IsMenuOpened('createDealer') then
            if WarMenu.ComboBox('Min. Time', times, currentMinTimeIndex, selectedMinTimeIndex, function(currentIndex, selectedIndex)
                ...
                N1Core.Functions.Notify('Time confirmed!', 'success')
            end
    ...
    while true do
        ...
        if showNames then
            for _, player in pairs(GetPlayersFromCoords(GetEntityCoords(GetPlayerPed(-1)), 5.0)) do
                ...
                N1Admin.Functions.DrawText3D(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z + 1.0, '['..PlayerId..'] '..PlayerName)
            end
        else
            Citizen.Wait(1000)
        end
	...
	while true do
        ...
        if deleteLazer then
            ...
            if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                ...
                N1Admin.Functions.DrawText3D(entityCoord.x, entityCoord.y, entityCoord.z, "Obj: " .. entity .. " Model: " .. GetEntityModel(entity).. " \nPress [~g~E~s~] to delete the object!", 2)
...
RegisterNetEvent('N1-admin:client:bringTp')
AddEventHandler('N1-admin:client:bringTp', function(coords)
    local ped = GetPlayerPed(-1)

    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent('N1-admin:client:Freeze')
AddEventHandler('N1-admin:client:Freeze', function(toggle)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('N1-admin:client:SendReport')
AddEventHandler('N1-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('N1-admin:server:SendReport', name, src, msg)
end)

RegisterNetEvent('N1-admin:client:SendStaffChat')
AddEventHandler('N1-admin:client:SendStaffChat', function(name, msg)
    TriggerServerEvent('N1-admin:server:StaffChatMessage', name, msg)
end)

RegisterNetEvent('N1-admin:client:SaveCar')
AddEventHandler('N1-admin:client:SaveCar', function()
    local ped = GetPlayerPed(-1)
    local veh = GetVehiclePedIsIn(ped)

    if veh ~= nil and veh ~= 0 then
        local plate = GetVehicleNumberPlateText(veh)
        local props = N1Core.Functions.GetVehicleProperties(veh)
        local hash = props.model
        if N1Core.Shared.VehicleModels[hash] ~= nil and next(N1Core.Shared.VehicleModels[hash]) ~= nil then
            TriggerServerEvent('N1-admin:server:SaveCar', props, N1Core.Shared.VehicleModels[hash], GetHashKey(veh), plate)
        else
            N1Core.Functions.Notify('You cant store this vehicle in your garage..', 'error')
        end
    else
        N1Core.Functions.Notify('You are not in a vehicle..', 'error')
    end
end)
...
RegisterNetEvent('N1-admin:client:SetModel')
AddEventHandler('N1-admin:client:SetModel', function(skin)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('N1-admin:client:SetSpeed')
AddEventHandler('N1-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('N1-weapons:client:SetWeaponAmmoManual')
AddEventHandler('N1-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = GetPlayerPed(-1)
    if weapon ~= "current" then
        local weapon = weapon:upper()
        SetPedAmmo(ped, GetHashKey(weapon), ammo)
        N1Core.Functions.Notify('+'..ammo..' Ammo for the '..N1Core.Shared.Weapons[GetHashKey(weapon)]["label"], 'success')
    else
        local weapon = GetSelectedPedWeapon(ped)
        if weapon ~= nil then
            SetPedAmmo(ped, weapon, ammo)
            N1Core.Functions.Notify('+'..ammo..' Ammo for the '..N1Core.Shared.Weapons[weapon]["label"], 'success')
        else
            N1Core.Functions.Notify('You dont have a weapon in your hands..', 'error')
        end
    end
end)

RegisterNetEvent('N1-admin:client:GiveNuiFocus')
AddEventHandler('N1-admin:client:GiveNuiFocus', function(focus, mouse)
