...
RegisterServerEvent('N1-traphouses:server:TakeoverHouse')
AddEventHandler('N1-traphouses:server:TakeoverHouse', function(Traphouse)
    local src = source
    ...
    if not HasCitizenIdHasKey(CitizenId, Traphouse) then
        if Player.Functions.RemoveMoney('cash', Config.TakeoverPrice) then
            TriggerClientEvent('N1-traphouses:client:TakeoverHouse', src, Traphouse)
        else
...
RegisterServerEvent('N1-traphouses:server:AddHouseKeyHolder')
AddEventHandler('N1-traphouses:server:AddHouseKeyHolder', function(CitizenId, TraphouseId, IsOwner)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    ...
    if Config.TrapHouses[TraphouseId] ~= nil then
        ...
        if Config.TrapHouses[TraphouseId].keyholders == nil then
            ...
            TriggerClientEvent('N1-traphouses:client:SyncData', -1, TraphouseId, Config.TrapHouses[TraphouseId])
        else
            if #Config.TrapHouses[TraphouseId].keyholders + 1 <= 6 then
                ...
                TriggerClientEvent('N1Core:Notify', src, 'There arent any slots over..')
            end
        end
    else
        TriggerClientEvent('N1Core:Notify', src, 'Error has occurred..')
    end
end)
...
N1Core.Commands.Add("entertraphouse", "Betreed traphouse", {}, false, function(source, args)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
 
    TriggerClientEvent('N1-traphouses:client:EnterTraphouse', src)
end)

N1Core.Commands.Add("givetrapkeys", "Give's the keys of the trap house", {{name = "id", help = "Speler id"}}, true, function(source, args)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
            ...
            if not HasCitizenIdHasKey(TargetData.PlayerData.citizenid, Traphouse) then
                if Config.TrapHouses[Traphouse] ~= nil then
                    ...
                    if Config.TrapHouses[Traphouse].keyholders == nil then
                        ...
                        TriggerClientEvent('N1-traphouses:client:SyncData', -1, Traphouse, Config.TrapHouses[Traphouse])
                    else
                        if #Config.TrapHouses[Traphouse].keyholders + 1 <= 6 then
                            ...
                            TriggerClientEvent('N1Core:Notify', src, 'there arent any slots left over..')
                        end
                    end
                else
                    TriggerClientEvent('N1Core:Notify', src, 'Error has occurred..')
                ...
                TriggerClientEvent('N1Core:Notify', src, 'This person already owns a key..', 'error')
            end
        else
            TriggerClientEvent('N1Core:Notify', src, 'You dont own the trap house or you dont own one at all..', 'error')
        end
    else
...
RegisterServerEvent('N1-traphouses:server:TakeMoney')
AddEventHandler('N1-traphouses:server:TakeMoney', function(TraphouseId)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    if Config.TrapHouses[TraphouseId].money ~= 0 then
        ...
        TriggerClientEvent('N1Core:Notify', src, 'There issent any money in the cupboard', 'error')
    end
end)
...
RegisterServerEvent('N1-traphouses:server:RobNpc')
AddEventHandler('N1-traphouses:server:RobNpc', function(Traphouse)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    ...
    if Chance == odd then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items["stickynote"], "add")
    else
...
N1Core.Functions.CreateCallback('N1-traphouses:server:GetTraphousesData', function(source, cb)
