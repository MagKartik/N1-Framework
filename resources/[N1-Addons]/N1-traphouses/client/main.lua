...
Citizen.CreateThread(function()
    ...
    if N1Core.Functions.GetPlayerData() ~= nil then
        ...
        N1Core.Functions.TriggerCallback('N1-traphouses:server:GetTraphousesData', function(trappies)
...
RegisterNetEvent('N1-traphouses:client:EnterTraphouse')
AddEventHandler('N1-traphouses:client:EnterTraphouse', function(code)
    if ClosestTraphouse ~= nil then
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        N1Core.Functions.Notify('This code is not valide..', 'error')
    end
end)
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('N1-traphouses:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
                ...
                if InteractDistance < 20 then
                    ...
                    if InteractDistance < 1 then
                        if not IsKeyHolder then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent('N1-traphouses:server:TakeoverHouse', CurrentTraphouse)
                            end
                            ...
                            if IsHouseOwner then
                                ...
                                if IsControlJustPressed(0, Keys["G"]) then
                                    N1Core.Functions.Notify('Pincode: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, Keys["H"]) then
                                local TraphouseInventory = {}
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent("N1-traphouses:server:TakeMoney", CurrentTraphouse)
                            end
                        end
                    end
                end
...
function EnterTraphouse(data)
    ...
    data = exports['N1-interior']:CreateTrevorsShell(coords)
    TraphouseObj = data[1]
    POIOffsets = data[2]
...
function LeaveTraphouse(data)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    exports['N1-interior']:DespawnInterior(TraphouseObj, function()
...
RegisterNetEvent('N1-traphouses:client:TakeoverHouse')
AddEventHandler('N1-traphouses:client:TakeoverHouse', function(TraphouseId)
    local ped = GetPlayerPed(-1)

    N1Core.Functions.Progressbar("takeover_traphouse", "Overtaking traphouse", math.random(1000, 3000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('N1-traphouses:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        N1Core.Functions.Notify("Overtaking canceled..", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            N1Core.Functions.Notify(CitizenId..' is added to the traphouse!')
        else
            N1Core.Functions.Notify(CitizenId..' This person does already have the keys!')
        end
    else
        N1Core.Functions.Notify('You can max give 6 other people acces to the trap house!')
    end
    IsKeyHolder = HasKey(CitizenId)
    IsHouseOwner = IsOwner(CitizenId)
end

RegisterNetEvent('N1-traphouses:client:SyncData')
AddEventHandler('N1-traphouses:client:SyncData', function(k, data)
