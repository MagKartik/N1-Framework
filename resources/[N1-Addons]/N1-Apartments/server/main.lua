...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    N1Core.Functions.ExecuteSql(false, "INSERT INTO `apartments` (`name`, `type`, `label`, `citizenid`) VALUES ('"..apartmentId.."', '"..type.."', '"..label.."', '"..Player.PlayerData.citizenid.."')")
    TriggerClientEvent('N1Core:Notify', src, "You got a apartment ("..label..")")
...
AddEventHandler('apartments:server:UpdateApartment', function(type)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    N1Core.Functions.ExecuteSql(false, "UPDATE `apartments` SET type='"..type.."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'")

    TriggerClientEvent('N1Core:Notify', src, "You have changed apartments")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
AddEventHandler('apartments:server:OpenDoor', function(target, apartmentId, apartment)
    local src = source
    local OtherPlayer = N1Core.Functions.GetPlayer(target)
...
function CreateApartmentId(type)
	...
	while not UniqueFound do
		...
		N1Core.Functions.ExecuteSql(true, "SELECT COUNT(*) as count FROM `apartments` WHERE `name` = '"..tostring(type .. AparmentId).."'", function(result)
...
function GetApartmentInfo(apartmentId)
    local retval = nil
    N1Core.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `name` = '"..apartmentId.."' ", function(result)
...
function GetOwnedApartment(citizenid)
    N1Core.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `citizenid` = '"..citizenid.."' ", function(result)
        if result[1] ~= nil then 
            return result[1]
        end
        return nil
...
N1Core.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
N1Core.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
N1Core.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
N1Core.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
        N1Core.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..cid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
        ...
        local src = source
        local Player = N1Core.Functions.GetPlayer(src)
        N1Core.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
...
N1Core.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    if Player ~= nil then
        N1Core.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
...
N1Core.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = N1Core.Functions.GetPlayer(src)
...
RegisterServerEvent('N1-Apartments:server:SetInsideMeta')
AddEventHandler('N1-Apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
