...
Citizen.CreateThread(function()
	while true do
		for key, doorID in ipairs(N1.Doors) do
...
Citizen.CreateThread(function()
	while true do
		...
		for k,doorID in ipairs(N1.Doors) do
			local distance
...
AddEventHandler('lockpicks:UseLockpick', function()
	...
	N1Core.Functions.TriggerCallback('N1-radio:server:GetItem', function(hasItem)
		for k, v in pairs(N1.Doors) do
			local dist = GetDistanceBetweenCoords(pos, N1.Doors[k].textCoords.x, N1.Doors[k].textCoords.y, N1.Doors[k].textCoords.z)
			if dist < 1.5 then
				if N1.Doors[k].pickable then
					if N1.Doors[k].locked then
						if hasItem then
							closestDoorKey, closestDoorValue = k, v
							TriggerEvent('N1-lockpick:client:openLockpick', lockpickFinish)
						else
							N1Core.Functions.Notify("You are missing a toolkit..", "error")
						end
					else
						N1Core.Functions.Notify('The door is already unlocked??', 'error', 2500)
					end
				else
					N1Core.Functions.Notify('The door lock is too strong', 'error', 2500)
				end
			end
		end
    end, "screwdriverset")
...
function lockpickFinish(success)
    if success then
		N1Core.Functions.Notify('Succes!', 'success', 2500)
		setDoorLocking(closestDoorValue, closestDoorKey)
    else
        N1Core.Functions.Notify('Failed..', 'error', 2500)
    end
end
...
function setDoorLocking(doorId, key)
    ...
    SetTimeout(400, function()
		...
		TriggerServerEvent('N1-doorlock:server:updateState', key, doorId.locked)
	end)
end
...
RegisterNetEvent('N1-doorlock:client:setState')
AddEventHandler('N1-doorlock:client:setState', function(doorID, state)
	N1.Doors[doorID].locked = state
end)

RegisterNetEvent('N1-doorlock:client:setDoors')
AddEventHandler('N1-doorlock:client:setDoors', function(doorList)
	N1.Doors = doorList
end)

RegisterNetEvent('N1Core:Client:OnPlayerLoaded')
AddEventHandler('N1Core:Client:OnPlayerLoaded', function()
    TriggerServerEvent("N1-doorlock:server:setupDoors")
end)