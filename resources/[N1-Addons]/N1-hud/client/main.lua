    ...
    while true do
        ...
        if (PlayerJob.name ~= "police") then
            if IsPlayerFreeAiming(PlayerId()) and not IsPedShooting(GetPlayerPed(-1)) then
                ...
                local WeaponData = N1Core.Shared.Weapons[CurrentWeapon]
    ...
    while true do
        if not IsGaining then
            ...
            if StressGain > 0 then
                N1Core.Functions.Notify('Gained stress', "primary", 2000)
                TriggerServerEvent('N1-hud:Server:UpdateStress', StressGain)
                StressGain = 0
            end
        end
...
function GetShakeIntensity(stresslevel)
    local retval = 0.05
    for k, v in pairs(N1Stress.Intensity["shake"]) do
...
function GetEffectInterval(stresslevel)
    local retval = 60000
    for k, v in pairs(N1Stress.EffectInterval) do
        if stresslevel >= v.min and stresslevel < v.max then
            retval = v.timeout
            break
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        elseif stress >= N1Stress.MinimumStress then
            local ShakeIntensity = GetShakeIntensity(stress)
            ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', ShakeIntensity)
