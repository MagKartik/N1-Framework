...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    while true do 
        if N1Core ~= nil and isLoggedIn and N1Hud.Show then
    ...
    while true do
        if N1Core ~= nil and isLoggedIn and N1Hud.Show then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                speed = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 3.6
                if speed >= N1Stress.MinimumSpeed then
                    TriggerServerEvent('N1-hud:Server:GainStress', math.random(1, 2))
                end
            end
        end
        Citizen.Wait(20000)
...
Citizen.CreateThread(function() 
    while true do
        Citizen.Wait(1000)
        if IsPedInAnyVehicle(PlayerPedId()) and isLoggedIn and N1Hud.Show then
            DisplayRadar(true)
            SendNUIMessage({
                action = "car",
                show = true,
...
RegisterNetEvent('N1-hud:client:ToggleHarness')
AddEventHandler('N1-hud:client:ToggleHarness', function(toggle)
    SendNUIMessage({
        action = "harness",
        toggle = toggle
    })
...
RegisterNetEvent('N1-hud:client:UpdateNitrous')
AddEventHandler('N1-hud:client:UpdateNitrous', function(toggle, level, IsActive)
    SendNUIMessage({
        action = "nitrous",
        toggle = toggle,
        level = level,
...
RegisterNetEvent('N1-hud:client:UpdateDrivingMeters')
AddEventHandler('N1-hud:client:UpdateDrivingMeters', function(toggle, amount)
    SendNUIMessage({
...
RegisterNetEvent('N1-hud:client:UpdateVoiceProximity')
AddEventHandler('N1-hud:client:UpdateVoiceProximity', function(Proximity)
    SendNUIMessage({
        action = "proximity",
        prox = Proximity
    })
...
RegisterNetEvent('N1-hud:client:ProximityActive')
AddEventHandler('N1-hud:client:ProximityActive', function(active)
    SendNUIMessage({
        action = "talking",
        IsTalking = active
    })
...
Citizen.CreateThread(function()
    while true do
        if isLoggedIn and N1Hud.Show and N1Core ~= nil then
            N1Core.Functions.TriggerCallback('hospital:GetPlayerBleeding', function(playerBleeding)
