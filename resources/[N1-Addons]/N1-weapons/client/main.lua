...
Citizen.CreateThread(function()
    ...
    if N1Core.Functions.GetPlayerData() ~= nil then
        ...
        N1Core.Functions.TriggerCallback("weapons:server:GetConfig", function(RepairPoints)
            ...
            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                if IsPedShooting(GetPlayerPed(-1)) or IsControlJustPressed(0, 24) then
                    if CanShoot then
                        ...
                        if N1Core.Shared.Weapons[weapon]["name"] == "weapon_snowball" then
                            TriggerServerEvent('N1Core:Server:RemoveItem', "snowball", 1)
                        else
                            if ammo > 0 then
                        ...
                        N1Core.Functions.Notify("This weapon is broken and can not be used..", "error")
                        MultiplierAmount = 0
                    end
                end
            end
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedShooting(ped) then
            ...
            if ammo - 1 < 1 then
                print('reset')
                SetAmmoInClip(GetPlayerPed(-1), GetHashKey(N1Core.Shared.Weapons[weapon]["name"]), 1)
            end
        end
...
AddEventHandler('weapon:client:AddAmmo', function(type, amount, itemData)
    ...
    if CurrentWeaponData ~= nil then
        if N1Core.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and N1Core.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            ...
            if (total + retval) <= (retval + 1) then
                N1Core.Functions.Progressbar("taking_bullets", "Loading bullets..", math.random(4000, 6000), false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    if N1Core.Shared.Weapons[weapon] ~= nil then
                        ...
                        TriggerServerEvent('N1Core:Server:RemoveItem', itemData.name, 1, itemData.slot)
                        TriggerEvent('inventory:client:ItemBox', N1Core.Shared.Items[itemData.name], "remove")
                        TriggerEvent('N1Core:Notify', retval.." Loading bullets!", "success")
                    end
                end, function()
                    N1Core.Functions.Notify("Canceled..", "error")
                end)
            else
                N1Core.Functions.Notify("Your weapon is already looded..", "error")
            end
        else
            N1Core.Functions.Notify("Your not holding a weapon..", "error")
        end
    else
        N1Core.Functions.Notify("Your not holding a weapon..", "error")
    end
                ...
                if distance < 10 then
                    ...
                    if distance < 1 then
                        if data.IsRepairing then
                            ...
                            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                                if not data.RepairingData.Ready then
                                    local WeaponData = N1Core.Shared.Weapons[GetHashKey(CurrentWeaponData.name)]
                                    ...
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        N1Core.Functions.TriggerCallback('weapons:server:RepairWeapon', function(HasMoney)
                                            if HasMoney then
                                                CurrentWeaponData = {}
                                            end
...
AddEventHandler("weapons:client:EquipAttachment", function(ItemData, attachment)
    local ped = GetPlayerPed(-1)
    local weapon = GetSelectedPedWeapon(ped)
    local WeaponData = N1Core.Shared.Weapons[weapon]
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        if Config.WeaponAttachments[WeaponData.name] ~= nil then
            if Config.WeaponAttachments[WeaponData.name][attachment] ~= nil then
                ...
                N1Core.Functions.Notify("This weapon does not support this attachment..", "error")
            end
        end
    else
        N1Core.Functions.Notify("You dont have a weapon in ur hand..", "error")
    end
end)
