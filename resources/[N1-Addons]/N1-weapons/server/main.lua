...
N1Core.Functions.CreateCallback("weapons:server:GetConfig", function(source, cb)
...
--     N1Core.Functions.ExecuteSql(false, "SELECT * FROM `playerammo` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
...
N1Core.Functions.CreateCallback("weapon:server:GetWeaponAmmo", function(source, cb, WeaponData)
...
N1Core.Functions.CreateUseableItem("pistol_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_PISTOL", 12, item)
end)

N1Core.Functions.CreateUseableItem("rifle_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_RIFLE", 30, item)
end)

N1Core.Functions.CreateUseableItem("smg_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SMG", 20, item)
end)

N1Core.Functions.CreateUseableItem("shotgun_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SHOTGUN", 10, item)
end)

N1Core.Functions.CreateUseableItem("mg_ammo", function(source, item)
...
--     local src = source
--     local Player = N1Core.Functions.GetPlayer(src)
--     local WeaponData = N1Core.Shared.Weapons[GetHashKey(data.name)]
...
--                 TriggerClientEvent('N1Core:Notify', src, "Your weapon is broken, u need to repair it before u can use it again.", "error")
--             end
--         else
            ...
            if WeaponSlot.info.quality ~= nil then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        WeaponSlot.info.quality = 0
                        TriggerClientEvent('inventory:client:UseWeapon', src, data)
                        TriggerClientEvent('N1Core:Notify', src, "Your weapon is broken, you need to repair it before you can use it again.", "error")
                        break
                    end
                end
            else
...
N1Core.Commands.Add("repairweapon", "Repair weapon command for staf", {{name="hp", help="HP of ur weapon"}}, true, function(source, args)
...
N1Core.Functions.CreateCallback("weapons:server:RepairWeapon", function(source, cb, RepairPoint, data)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    local minute = 60 * 1000
    local Timeout = math.random(5 * minute, 10 * minute)
    local WeaponData = N1Core.Shared.Weapons[GetHashKey(data.name)]
    local WeaponClass = (N1Core.Shared.SplitStr(WeaponData.ammotype, "_")[2]):lower()
    ...
    if Player.PlayerData.items[data.slot] ~= nil then
        if Player.PlayerData.items[data.slot].info.quality ~= nil then
            if Player.PlayerData.items[data.slot].info.quality ~= 100 then
                if Player.Functions.RemoveMoney('cash', Config.WeaponRepairCotsts[WeaponClass]) then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items[data.name], "remove")
                    ...
                    SetTimeout(Timeout, function()
                        ...
                        TriggerClientEvent('weapons:client:SyncRepairShops', -1, Config.WeaponRepairPoints[RepairPoint], RepairPoint)
                        TriggerEvent('N1-phone:server:sendNewMailToOffline', Player.PlayerData.citizenid, {
                            sender = "Tyrone",
                            subject = "Reperatie",
                ...
                TriggerClientEvent("N1Core:Notify", src, "This weapon is not dammaged..", "error")
                cb(false)
            end
        else
            TriggerClientEvent("N1Core:Notify", src, "This weapon is not dammaged..", "error")
        ...
        TriggerClientEvent('N1Core:Notify', src, "You diddent have a weapon in ur hands..", "error")
...
AddEventHandler("weapons:server:TakeBackWeapon", function(k, data)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items[itemdata.name], "add")
    Config.WeaponRepairPoints[k].IsRepairing = false
    Config.WeaponRepairPoints[k].RepairingData = {}
    TriggerClientEvent('weapons:client:SyncRepairShops', -1, Config.WeaponRepairPoints[k], k)
end)

N1Core.Functions.CreateUseableItem("pistol_suppressor", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

N1Core.Functions.CreateUseableItem("smg_suppressor", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

N1Core.Functions.CreateUseableItem("rifle_suppressor", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

N1Core.Functions.CreateUseableItem("pistol_extendedclip", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

N1Core.Functions.CreateUseableItem("smg_extendedclip", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

N1Core.Functions.CreateUseableItem("smg_flashlight", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "flashlight")
end)

N1Core.Functions.CreateUseableItem("smg_scope", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
...
N1Core.Functions.CreateUseableItem("rifle_extendedclip", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

N1Core.Functions.CreateUseableItem("rifle_drummag", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
    ...
    if Inventory[CurrentWeaponData.slot] ~= nil then
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("N1Core:Notify", src, "You already have "..AttachmentData.label:lower().." on your weapon.", "error", 3500)
            end
        else
...
N1Core.Functions.CreateCallback('weapons:server:RemoveAttachment', function(source, cb, AttachmentData, ItemData)
    local src = source
    local Player = N1Core.Functions.GetPlayer(src)
    local Inventory = Player.PlayerData.items
    ...
    if Inventory[ItemData.slot] ~= nil then
        if Inventory[ItemData.slot].info.attachments ~= nil and next(Inventory[ItemData.slot].info.attachments) ~= nil then
            ...
            if HasAttach then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items[AttachmentComponent.item], "add")
                TriggerClientEvent("N1Core:Notify", src, "You removed "..AttachmentComponent.label.." from your weapon!", "error")
