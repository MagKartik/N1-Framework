...
AddEventHandler('qb-vehicleshop:server:buyVehicle', function(vehicleData, garage)
    ...
    if (balance - vData["price"]) >= 0 then
        ...
        TriggerClientEvent("QBCore:Notify", src, "Weldone! Your vehicle is deliverd "..QB.GarageLabel[garage], "success", 5000)
		...
		TriggerClientEvent("QBCore:Notify", src, "You dont have enough money, you miss ₹"..format_thousand(vData["price"] - balance), "error", 5000)
    end
end)
...
AddEventHandler('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle, class)
    ...
    if (balance - vehiclePrice) >= 0 then
        ...
        TriggerClientEvent("QBCore:Notify", src, "Weldone! Your vehicle is waiting outside for you.", "success", 5000)
        ...
        TriggerClientEvent("QBCore:Notify", src, "You dont have enough money, you miss ₹"..format_thousand(vehiclePrice - balance), "error", 5000)
    end
end)
...
QBCore.Commands.Add("verkoop", "Sell vehicle out of custom car dealer", {}, false, function(source, args)
    ...
    if Player.PlayerData.job.name == "cardealer" then
        if TargetId ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', source, 'You need to giveaway player id', 'error')
        end
    else
...
QBCore.Commands.Add("testdrive", "Take a test drive", {}, false, function(source, args)
    ...
    if Player.PlayerData.job.name == "cardealer" then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'You arrent a car dealer', 'error')
    end
end)
...
AddEventHandler('qb-vehicleshop:server:ConfirmVehicle', function(ShowroomVehicle)
    ...
    elseif Player.PlayerData.money.bank >= VehPrice then
        ...
        if Player.PlayerData.money.cash > Player.PlayerData.money.bank then
            TriggerClientEvent('QBCore:Notify', src, 'You dont have enough money.. you mis ('..(Player.PlayerData.money.cash - VehPrice)..',-)')
        else
            TriggerClientEvent('QBCore:Notify', src, 'You dont have enough money.. you mis ('..(Player.PlayerData.money.bank - VehPrice)..',-)')
        end
    end
end)
