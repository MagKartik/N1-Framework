...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            QBCore.Functions.Notify('You ar to far from your hotdog stall..', 'error')
        end
    else
        QBCore.Functions.Notify('You have no hotdog stall', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 15.0 then
            ...
            QBCore.Functions.Notify('You're to far from the stall..', 'error')
            break
        end
        Citizen.Wait(100)
    end
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    QBCore.Functions.Notify('Custommer refused!', 'error')
                    SellingData.HasTarget = false
                ...
                SellingData.Hotdog = GetAvailableHotdog()
                QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, 'You have no hotdogs.. / [8] Refuse cutommer')
            ...
            QBCore.Functions.Notify('You're to far from the stal..', 'error')
            break
        end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        ...
        if Config.MyLevel == 1 then
            QBCore.Functions.Notify('You made a  '..Config.Stock[Quality].Label..' Hotdog !')
            ...
            if Luck == LuckyNumber then
                QBCore.Functions.Notify('You have '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s !')
        ...
        QBCore.Functions.Notify(You have  ('..Config.Stock[Quality].Label..') no hotdogs..')
    end
    PreparingFood = false
end
    ...
    if DoesEntityExist(StandObject) then
        QBCore.Functions.TriggerCallback('qb-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                QBCore.Functions.Notify('You recieved your 1000,- borg!', 'success')
            else
                QBCore.Functions.Notify('Something is wrong meine freunden..', 'error')
            end
        end)
    else
        QBCore.Functions.Notify('Your stall is no where to see you wont recieve you're deposit', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
AddEventHandler('qb-hotdogjob:staff:DeletStand', function()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            QBCore.Functions.Notify('Hotdog stal removed!')
        end
    end
end)
