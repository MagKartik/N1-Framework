...
function enterOwnedHouse(house)
    ...
    elseif Config.Houses[house].tier == 8 then
        data = exports['qb-interior']:CreateGarageboxShell(coords)
    end
            ...
            if IsPresent then
                ...
                if dist < 1 then
                    if Config.Houses[closesthouse].locked then
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('qb-houses:server:lockHouse', false, closesthouse)
                                    QBCore.Functions.Notify('Het is gelukt, de deur ligt er nu uit.', 'success')
                                ...
                                QBCore.Functions.Notify('Het is mislukt.. Probeer het nogmaals.', 'error')
                                DoRamAnimation(false)
                            end)
                        ...
                        QBCore.Functions.Notify('Dit huis is al open..', 'error')
                    end
                else
                    QBCore.Functions.Notify('Je bent niet bij een huis in de buurt..', 'error')
                end
            else
                QBCore.Functions.Notify('Er is geen korpsleiding aanwezig..', 'error')
            end
        end)
    else
        QBCore.Functions.Notify('Je bent niet bij een huis in de buurt..', 'error')
...
AddEventHandler('qb-houses:client:ResetHouse', function()
    ...
    if closesthouse ~= nil then
        ...
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            QBCore.Functions.Notify('Je hebt het huis weer opslot gedaan..', 'success')
        else
            QBCore.Functions.Notify('Deze deur is niet open gebroken..', 'error')
        end
    end
end)