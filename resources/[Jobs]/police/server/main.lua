...
AddEventHandler('police:server:CheckBills', function()
    ...
    N1Core.Functions.ExecuteSql(false, "SELECT * FROM `bills` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `type` = 'police'", function(result)
        if result[1] ~= nil then
            ...
            N1Core.Functions.ExecuteSql(false, "DELETE FROM `bills` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `type` = 'police'")
            TriggerClientEvent('police:client:sendBillingMail', src, totalAmount)
            TriggerEvent('N1-moneysafe:server:DepositMoney', "police", totalAmount, "bills")
		end
	end)
end)
...
AddEventHandler('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    local CuffedPlayer = N1Core.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:EscortPlayer', function(playerId)
    local src = source
    local Player = N1Core.Functions.GetPlayer(source)
    local EscortPlayer = N1Core.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('N1Core:Notify', OtherPlayer.PlayerData.source, "You received a fine of ₹"..price)
        end
    end
end)
...
AddEventHandler('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('N1Core:Notify', src, "You sent the person to prison for "..time.." months")
        end
    end
end)
...
AddEventHandler('police:server:SendEmergencyMessage', function(coords, message)
    local src = source
    local MainPlayer = N1Core.Functions.GetPlayer(src)
    local alertData = {
...
AddEventHandler('police:server:SearchPlayer', function(playerId)
    local src = source
    local SearchedPlayer = N1Core.Functions.GetPlayer(playerId)
    if SearchedPlayer ~= nil then 
        TriggerClientEvent('chatMessage', source, "SYSTEM", "warning", "Person has ₹"..SearchedPlayer.PlayerData.money["cash"]..",- on him..")
        TriggerClientEvent('N1Core:Notify', SearchedPlayer.PlayerData.source, "You are being searched..")
    end
end)
...
AddEventHandler('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, N1Core.Shared.Items["moneybag"], "add")
        TriggerClientEvent('N1Core:Notify', SearchedPlayer.PlayerData.source, "Your cash is confiscated..")
    end
end)
...
AddEventHandler('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('N1Core:Notify', SearchedPlayer.PlayerData.source, "Your driving license has been confiscated..")
        else
            TriggerClientEvent('N1Core:Notify', src, "Can't confiscate driving license..", "error")
        end
    end
end)
...
AddEventHandler('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('N1Core:Notify', SearchedPlayer.PlayerData.source, "You have been robbed of ₹"..money.."..")
    end
end)
...
AddEventHandler('police:server:Impound', function(plate, fullImpound, price)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('N1Core:Notify', src, "Vehicle taken into depot for ₹"..price.."!")
        else
            exports['ghmattimysql']:execute('UPDATE player_vehicles SET state = @state WHERE plate = @plate', {['@state'] = 2, ['@plate'] = plate})
            TriggerClientEvent('N1Core:Notify', src, "Vehicle completely seized!")
        end
    end
end)
...
AddEventHandler('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        if Player.Functions.AddItem("filled_evidence_bag", 1, false, bloodInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, N1Core.Shared.Items["filled_evidence_bag"], "add")
            TriggerClientEvent("evidence:client:RemoveBlooddrop", -1, bloodId)
            BloodDrops[bloodId] = nil
        end
    else
        TriggerClientEvent('N1Core:Notify', src, "You must have an empty evidence bag with you", "error")
    end
end)
...
AddEventHandler('evidence:server:CreateCasing', function(weapon, coords)
    ...
    local weaponInfo = N1Core.Shared.Weapons[weapon]
...
AddEventHandler('police:server:showFingerprint', function(playerId)
    local src = source
    local Player = N1Core.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:SetTracker', function(targetId)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('N1Core:Notify', targetId, 'Your anklet is taken off.', 'error', 5000)
        TriggerClientEvent('N1Core:Notify', source, 'You took off an ankle bracelet from '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData("tracker", true)
        TriggerClientEvent('N1Core:Notify', targetId, 'You put on an ankle strap.', 'error', 5000)
        TriggerClientEvent('N1Core:Notify', source, 'You put on an ankle strap to '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
...
AddEventHandler('police:server:SendTrackerLocation', function(coords, requestId)
    local Target = N1Core.Functions.GetPlayer(source)
    local TrackerMeta = Target.PlayerData.metadata["tracker"]
    ...
    TriggerClientEvent("N1-phone:client:addPoliceAlert", requestId, alertData)
end)
...
N1Core.Functions.CreateCallback('police:server:isPlayerDead', function(source, cb, playerId)
    local Player = N1Core.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

N1Core.Functions.CreateCallback('police:GetPlayerStatus', function(source, cb, playerId)
    local Player = N1Core.Functions.GetPlayer(playerId)
    local statList = {}
	if Player ~= nil then
...
N1Core.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = N1Core.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(N1Core.Shared.Weapons[weapon]["name"])
    local retval = false
...
N1Core.Functions.CreateCallback('police:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    for k, v in pairs(N1Core.Functions.GetPlayers()) do
        local Player = N1Core.Functions.GetPlayer(v)
        if Player ~= nil then 
...
N1Core.Functions.CreateCallback('police:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
N1Core.Functions.CreateCallback('police:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
N1Core.Functions.CreateCallback('police:GetCops', function(source, cb)
    local amount = 0
...
N1Core.Commands.Add("setpolice", "Give the police job to someone ", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = N1Core.Functions.GetPlayer(tonumber(args[1]))
    local Myself = N1Core.Functions.GetPlayer(source)
    if Player ~= nil then 
...
N1Core.Commands.Add("spikestrip", "Place a spike strip", {}, false, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    if Player ~= nil then 
        if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
...
N1Core.Commands.Add("firepolice", "Fire a police officer!", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = N1Core.Functions.GetPlayer(tonumber(args[1]))
    local Myself = N1Core.Functions.GetPlayer(source)
    if Player ~= nil then 
        if (Myself.PlayerData.job.name == "police" and Myself.PlayerData.job.onduty) and IsHighCommand(Myself.PlayerData.citizenid) then
...
N1Core.Commands.Add("pobject", "Place/Delete an object", {{name="type", help="Type object you want or 'delete' to delete"}}, true, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
...
N1Core.Commands.Add("cuff", "Cuff a player", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Commands.Add("palert", "Make a police alert", {{name="alert", help="The Police alert"}}, false, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        if args[1] ~= nil then
            ...
            TriggerEvent("N1-log:server:CreateLog", "palert", "Police alert", "blue", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Alert:** " ..msg, false)
...
N1Core.Commands.Add("escort", "Escort a person", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    TriggerClientEvent("police:client:EscortPlayer", source)
end)

N1Core.Commands.Add("mdt", "Toggle police mdt", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Commands.Add("callsign", "Put the name of your callsign (call number)", {{name="name", help="Name of your callsign"}}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    Player.Functions.SetMetaData("callsign", table.concat(args, " "))
end)

N1Core.Commands.Add("clearcasings", "Take away bullet casings in the area (make sure you have picked up some)", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Commands.Add("jail", "Jail a person", {{name="id", help="Player ID"},{name="time", help="Time they have to be in jail"}}, true, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Commands.Add("unjail", "Unjail a person.", {{name="id", help="Player ID"}}, true, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        local playerId = tonumber(args[1])
...
N1Core.Commands.Add("gimme", ":)", {}, false, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    local info = {
...
N1Core.Commands.Add("gimme2", ":)", {}, false, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    local info = {
        serie = "K"..math.random(10,99).."SH"..math.random(100,999).."HJ"..math.random(1,9),
        attachments = {
...
N1Core.Commands.Add("clearblood", "Take away nearby blood (make sure you've picked up some)", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Commands.Add("seizecash", "Take cash from the nearest person", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
N1Core.Commands.Add("sc", "Handcuff someone but he can walk", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Commands.Add("cam", "View security camera", {{name="camid", help="Camera ID"}}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Commands.Add("flagplate", "Flag a vehicle", {{name="plate", help="License"}, {name="reason", help="Reason of flagging the vehicle"}}, true, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('N1Core:Notify', source, "Vehicle ("..args[1]:upper()..") is flagged for: "..table.concat(reason, " "))
    else
        TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "This command is for emergency services!")
    end
end)

N1Core.Commands.Add("unflagplate", "Unflag a vehicle", {{name="plate", help="License plate"}}, true, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
                Plates[args[1]:upper()].isflagged = false
                TriggerClientEvent('N1Core:Notify', source, "Vehicle ("..args[1]:upper()..") is unflagged")
            else
...
N1Core.Commands.Add("plateinfo", "Flag a vehicle", {{name="plate", help="License plate"}}, true, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
...
N1Core.Commands.Add("depot", "Send a vehicle to the depot", {{name="prijs", help="Price for how much the person has to pay (may be empty)"}}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Commands.Add("impound", "Impound a vehicle", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Commands.Add("paytow", "Pay a bergnet worker", {{name="id", help="ID of the player"}}, true, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" then
        ...
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "tow" then
                ...
                TriggerClientEvent('N1Core:Notify', source, 'You paid a bergnet worker')
            else
                TriggerClientEvent('N1Core:Notify', source, 'Person is not a bergnet worker', "error")
            end
        end
    else
        TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "This command is for emergency services!")
...
N1Core.Commands.Add("paylaw", "Pay a lawyer", {{name="id", help="ID of the player"}}, true, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        ...
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "lawyer" then
                ...
                TriggerClientEvent('N1Core:Notify', source, 'You paid a lawyer')
            else
                TriggerClientEvent('N1Core:Notify', source, 'Person is not a lawyer', "error")
            end
        end
    else
        TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "This command is for emergency services!")
...
N1Core.Commands.Add("radar", "Toggle speedradar :)", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
N1Core.Functions.CreateUseableItem("handcuffs", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
...
N1Core.Commands.Add("911", "Send a report to emergency services", {{name="message", help="Message you want to send"}}, true, function(source, args)
    ...
    if Player.Functions.GetItemByName("phone") ~= nil then
        ...
        TriggerEvent("N1-log:server:CreateLog", "911", "911 alert", "blue", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Alert:** " ..message, false)
    else
        TriggerClientEvent('N1Core:Notify', source, 'You dont have a phone', 'error')
    end
end)

N1Core.Commands.Add("911a", "Send an anonymous report to emergency services (gives no location)", {{name="message", help="Message you want to send"}}, true, function(source, args)
    local message = table.concat(args, " ")
    local Player = N1Core.Functions.GetPlayer(source)
...
N1Core.Commands.Add("911r", "Send a message back to a alert", {{name="id", help="ID of the alert"}, {name="bericht", help="Message you want to send"}}, true, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    local OtherPlayer = N1Core.Functions.GetPlayer(tonumber(args[1]))
    table.remove(args, 1)
    local message = table.concat(args, " ")
    local Prefix = "POLICE"
...
N1Core.Commands.Add("anklet", "Put an anklet on the nearest person.", {}, false, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
...
N1Core.Commands.Add("ankletlocation", "remove an anklet off the nearest person", {{"bsn", "BSN of person"}}, true, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    
    if Player.PlayerData.job.name == "police" then
        if args[1] ~= nil then
            local citizenid = args[1]
            local Target = N1Core.Functions.GetPlayerByCitizenId(citizenid)
            ...
            if Target ~= nil then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('N1Core:Notify', source, 'This person does not have an anklet.', 'error')
                end
            end
        end
    else
...
N1Core.Commands.Add("ebutton", "Send a message back to a notification", {}, false, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") and Player.PlayerData.job.onduty) then
...
N1Core.Commands.Add("takedrivinglicense", "Take the driving license from someone", {}, false, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) then
...
N1Core.Commands.Add("takedna", "Take a DNA sanple from a person (empty evidence bag needed)", {{"id", "ID of the person"}}, true, function(source, args)
    local Player = N1Core.Functions.GetPlayer(source)
    local OtherPlayer = N1Core.Functions.GetPlayer(tonumber(args[1]))
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer ~= nil then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            if Player.Functions.AddItem("filled_evidence_bag", 1, false, info) then
                TriggerClientEvent("inventory:client:ItemBox", source, N1Core.Shared.Items["filled_evidence_bag"], "add")
            end
        else
            TriggerClientEvent('N1Core:Notify', source, "You must have an empty evidence bag with you", "error")
        end
    end
end)

N1Core.Functions.CreateUseableItem("moneybag", function(source, item)
    local Player = N1Core.Functions.GetPlayer(source)
    if Player.Functions.GetItemBySlot(item.slot) ~= nil then
...
N1Core.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(source, cb)
    local retval = false
    for k, v in pairs(N1Core.Functions.GetPlayers()) do
        local Player = N1Core.Functions.GetPlayer(v)
        if Player ~= nil then 
