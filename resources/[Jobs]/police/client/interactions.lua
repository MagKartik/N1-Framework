Citizen.CreateThread(function()
    while true do 
        ...
        if isHandcuffed then
            ...
            if (not IsEntityPlayingAnim(GetPlayerPed(-1), "mp_arresting", "idle", 3) and not IsEntityPlayingAnim(GetPlayerPed(-1), "mp_arrest_paired", "crook_p2_back_right", 3)) and not N1Core.Functions.GetPlayerData().metadata["isdead"] then
...
AddEventHandler('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            N1Core.Functions.Progressbar("robbing_player", "Robbing person..", math.random(5000, 7000), false, true, {
...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            N1Core.Functions.Notify("Time must be higher than 0..", "error")
        end
    else
        N1Core.Functions.Notify("No one nearby!", "error")
    end
    ...
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(GetPlayerPed(-1))) then
                ...
                N1Core.Functions.Notify("You cant cuff someone in a vehicle", "error")
            end
        else
            N1Core.Functions.Notify("No one nearby!", "error")
        end
            ...
            N1Core.Functions.TriggerCallback('N1Core:HasItem', function(result)
                if result then 
                    ...
                    if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(GetPlayerPed(-1))) then
                        ...
                        N1Core.Functions.Notify("You can\'t cuff someone in a vehicle", "error")
                    end
                else
                    N1Core.Functions.Notify("You don\'t have handcuffs on you", "error")
                end
            end, "handcuffs")
        else
            N1Core.Functions.Notify("No one nearby!", "error")
...
AddEventHandler('police:client:GetKidnappedDragger', function(playerId)
    N1Core.Functions.GetPlayerData(function(PlayerData)
        ...
        TriggerEvent('N1-kidnapping:client:SetKidnapping', isEscorting)
    end)
end)
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            N1Core.Functions.Notify("You are cuffed!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            N1Core.Functions.Notify("You are cuffed, but you can walk")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(GetPlayerPed(-1))
        N1Core.Functions.Notify("You are uncuffed!")
    end
end)
...
function IsTargetDead(playerId)
    local retval = false
    N1Core.Functions.TriggerCallback('police:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Citizen.Wait(100)
    return retval
