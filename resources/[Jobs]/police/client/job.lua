                    ...
                    if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 5) then
                        if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                ...
                                TriggerEvent('N1-policealerts:ToggleDuty', onDuty)
                            end
                            ...
                            if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                                        ...
                                        N1Core.Functions.SpawnVehicle(Config.Helicopter, function(veh)
                    ...
                    if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 4.5) then
                        if onDuty then
                            if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_"..N1Core.Functions.GetPlayerData().citizenid)
                                    TriggerEvent("inventory:client:SetCurrentStash", "policestash_"..N1Core.Functions.GetPlayerData().citizenid)
                                end
                            elseif (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 2.5) then
    ...
    N1Core.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                N1Core.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result ~= nil then
...
function ImpoundVehicleList()
    N1Core.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ped = GetPlayerPed(-1);
        MenuTitle = "Vehicles:"
        ClearMenu()

        if result == nil then
            N1Core.Functions.Notify("There are no impounded vehicles", "error", 5000)
            closeMenuFull()
        else
            for k, v in pairs(result) do
                ...
                Menu.addButton(N1Core.Shared.Vehicles[v.vehicle]["name"], "TakeOutImpound", v, "Impounded", " Engine: " .. enginePercent .. "%", " Body: " .. bodyPercent.. "%", " Fuel: "..currentFuel.. "%")
            end
        end
...
function OutfitsLijst()
    N1Core.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ...
        if outfits == nil then
            N1Core.Functions.Notify("You have no outfits saved...", "error", 3500)
            closeMenuFull()
        else
...
function removeOutfit(oData)
    ...
    N1Core.Functions.Notify(oData.outfitname.." is deleted", "success", 2500)
    closeMenuFull()
end
...
function IsArmoryWhitelist()
    local retval = false
    local citizenid = N1Core.Functions.GetPlayerData().citizenid
