...
AddEventHandler('hospital:server:RespawnAtHospital', function()
	...
	for k, v in pairs(Config.Locations["beds"]) do
		...
		N1Core.Functions.ExecuteSql(true, "UPDATE `players` SET `inventory` = '"..N1Core.EscapeSqli(json.encode({})).."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'")
		Player.Functions.RemoveMoney("bank", Config.BillCost, "respawned-at-hospital")
		TriggerClientEvent('N1Core:Notify', src, 'All your possessions have been taken..', 'error')
...
AddEventHandler('hospital:server:TreatWounds', function(playerId)
	local src = source
	local Player = N1Core.Functions.GetPlayer(src)
	local Patient = N1Core.Functions.GetPlayer(playerId)
	if Patient ~= nil then
	...
	if Patient ~= nil then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('N1Core:Notify', src, "You don\'t have enough money on you..", "error")
			end
		else
			TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
		end
...
AddEventHandler('hospital:server:MakeDeadCall', function(blipSettings, gender, street1, street2)
	...
	if street2 ~= nil then
		...
		TriggerClientEvent('N1-policealerts:client:AddPoliceAlert', -1, {
            timeOut = 5000,
...
N1Core.Functions.CreateCallback('hospital:GetDoctors', function(source, cb)
	local amount = 0
	for k, v in pairs(N1Core.Functions.GetPlayers()) do
		local Player = N1Core.Functions.GetPlayer(v)
		if Player ~= nil then 
...
N1Core.Functions.CreateCallback('hospital:GetPlayerStatus', function(source, cb, playerId)
...
N1Core.Functions.CreateCallback('hospital:GetPlayerBleeding', function(source, cb)
	local src = source
...
N1Core.Commands.Add("status", "Check a person his health", {}, false, function(source, args)
...
N1Core.Commands.Add("heal", "Help a person his injuries", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "doctor" or Player.PlayerData.job.name == "ambulance" then
...
N1Core.Commands.Add("revivep", "Revive a person", {}, false, function(source, args)
	local Player = N1Core.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "doctor" then
...
N1Core.Commands.Add("revive", "Revive a player or yourself", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
...
N1Core.Commands.Add("setpain", "Set pain to a player or yourself", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
		local Player = N1Core.Functions.GetPlayer(tonumber(args[1]))
		if Player ~= nil then
...
N1Core.Commands.Add("kill", "Kill a player or yourself", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
		local Player = N1Core.Functions.GetPlayer(tonumber(args[1]))
		if Player ~= nil then
...
N1Core.Commands.Add("setambulance", "Give the ambulance job to someone ", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = N1Core.Functions.GetPlayer(tonumber(args[1]))
...
N1Core.Commands.Add("setdoctor", "Give the doctor job to someone ", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = N1Core.Functions.GetPlayer(tonumber(args[1]))
    local Myself = N1Core.Functions.GetPlayer(source)
    if Player ~= nil then 
        if ((Myself.PlayerData.job.name == "ambulance" or Myself.PlayerData.job.name == "doctor") and Myself.PlayerData.job.onduty) and IsHighCommand(Myself.PlayerData.citizenid) then
...
N1Core.Functions.CreateUseableItem("bandage", function(source, item)
	local Player = N1Core.Functions.GetPlayer(source)
...
N1Core.Functions.CreateUseableItem("painkillers", function(source, item)
	local Player = N1Core.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
N1Core.Functions.CreateUseableItem("firstaid", function(source, item)
	local Player = N1Core.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
AddEventHandler('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('N1Core:Notify', helperId, "You can\'t help this person..", "error")
	end
end)