        ...
        if N1Core ~= nil then
            ...
            if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, true) < 1.5) then
                if doctorCount >= Config.MinimalDoctors then
                    N1Core.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Call doctor")
                else
                    N1Core.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Check in")
                end
                if IsControlJustReleased(0, Keys["E"]) then
                    if doctorCount >= Config.MinimalDoctors then
                        ...
                        N1Core.Functions.Progressbar("hospital_checkin", "Checking in..", 2000, false, true, {
                            ...
                            if bedId ~= nil then 
                                ...
                                N1Core.Functions.Notify("Beds are occupied..", "error")
                            end
                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                            N1Core.Functions.Notify("Checking in failed!", "error")
                        end)
                    end
                end
            elseif (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, true) < 4.5) then
                if doctorCount >= Config.MinimalDoctors then
                    N1Core.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Call")
                else
                    N1Core.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Check in")
                end
            end
            ...
            if closestBed ~= nil and not isInHospitalBed then
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["beds"][closestBed].x, Config.Locations["beds"][closestBed].y, Config.Locations["beds"][closestBed].z, true) < 1.5) then
                    N1Core.Functions.DrawText3D(Config.Locations["beds"][closestBed].x, Config.Locations["beds"][closestBed].y, Config.Locations["beds"][closestBed].z + 0.3, "~g~E~w~ - To lie in bed")
                    if IsControlJustReleased(0, Keys["E"]) then
    ...
    while true do
        ...
        if N1Core ~= nil then
            if isInHospitalBed and canLeaveBed then
                local pos = GetEntityCoords(GetPlayerPed(-1))
                N1Core.Functions.DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - To get out of bed..")
                if IsControlJustReleased(0, Keys["E"]) then
                    LeaveBed()
                end
            else
...
AddEventHandler('hospital:client:Revive', function()
    ...
    TriggerServerEvent('N1-hud:Server:RelieveStress', 100)
    TriggerServerEvent("hospital:server:SetDeathStatus", false)
    TriggerServerEvent("hospital:server:SetLaststandStatus", false)
    
    N1Core.Functions.Notify("You are completely healthy again!")
end)
...
AddEventHandler('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    N1Core.Functions.Notify("Your wounds have been healed!")
end)
...
AddEventHandler('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    Citizen.CreateThread(function ()
        ...
        local player = PlayerPedId()
        if isRevive then
            N1Core.Functions.Notify("You are being helped..")
...
AddEventHandler('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        local charinfo = N1Core.Functions.GetPlayerData().charinfo
        TriggerServerEvent('N1-phone:server:sendNewMail', {
            sender = "Pillbox",
            subject = "Hospital Costs",
            message = "Dear " .. genHospital Costsder .. " " .. charinfo.lastname .. ",<br /><br />Hereby you received an email with the costs of the last hospital visit.<br />The final costs have become: <strong>₹"..amount.."</strong><br /><br />We wish you a quick recovery!",
            button = {}
        })
    end)
end)
...
function DoLimbAlert()
    ...
    if not isDead then
        if #injured > 0 then
            ...
            N1Core.Functions.Notify(limbDamageMsg, "primary", 5000)
        end
    end
end
...
function DoBleedAlert()
    ...
    if not isDead and tonumber(isBleeding) > 0 then
        N1Core.Functions.Notify("You are "..Config.BleedingStates[tonumber(isBleeding)].label, "error", 5000)
    end
end
...
function ResetAll()
    ...
    TriggerServerEvent("N1Core:Server:SetMetaData", "hunger", 100)
    TriggerServerEvent("N1Core:Server:SetMetaData", "thirst", 100)
end
