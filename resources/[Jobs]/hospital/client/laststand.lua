        ...
        Citizen.CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    N1Core.Functions.Notify("You have bled out..", "error")
                    SetLaststand(false)
                    ...
                    local weaponLabel = N1Core.Shared.Weapons[killerWeapon] ~= nil and N1Core.Shared.Weapons[killerWeapon]["label"] or "Unknown"
                    local weaponName = N1Core.Shared.Weapons[killerWeapon] ~= nil and N1Core.Shared.Weapons[killerWeapon]["name"] or "Unknown_Weapon"
                    TriggerServerEvent("N1-log:server:CreateLog", "death", GetPlayerName(player) .. " ("..GetPlayerServerId(player)..") is dead", "red", "**".. killerName .. "** has killed  ".. GetPlayerName(player) .." with a **".. weaponLabel .. "** (" .. weaponName .. ")")
                    deathTime = 0
                    OnDeath()
                    DeathTimer()
                end
...
AddEventHandler('hospital:client:UseFirstAid', function()
    if not isEscorting then
        ...
        N1Core.Functions.Notify('Action impossible!', 'error')
    end
end)
...
AddEventHandler('hospital:client:HelpPerson', function(targetId)
    ...
    N1Core.Functions.Progressbar("hospital_revive", "Reviving person..", math.random(30000, 60000), false, true, {
        ...
        ClearPedTasks(ped)
        N1Core.Functions.Notify("You revived a person.")
