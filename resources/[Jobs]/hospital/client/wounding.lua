            ...
            if bleedTickTimer >= Config.BleedTickRate and not isInHospitalBed then
                if not isDead and not InLaststand then
                    if isBleeding > 0 then
                        ...
                        TriggerServerEvent("evidence:server:CreateBloodDrop", N1Core.Functions.GetPlayerData().citizenid, N1Core.Functions.GetPlayerData().metadata["bloodtype"], coords)
...
AddEventHandler('hospital:client:UseBandage', function()
    N1Core.Functions.Progressbar("use_bandage", "Using bandage..", 4000, false, true, {
        ...
        TriggerServerEvent("N1Core:Server:RemoveItem", "bandage", 1)
        TriggerEvent("inventory:client:ItemBox", N1Core.Shared.Items["bandage"], "remove")
...
AddEventHandler('hospital:client:UsePainkillers', function()
    N1Core.Functions.Progressbar("use_bandage", "Taking painkillers", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent("N1Core:Server:RemoveItem", "painkillers", 1)
        TriggerEvent("inventory:client:ItemBox", N1Core.Shared.Items["painkillers"], "remove")
        onPainKillers = true
