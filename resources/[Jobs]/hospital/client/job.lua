...
AddEventHandler('hospital:client:AiCall', function()
    ...
    local gender = N1Core.Functions.GetPlayerData().gender
    ...
    N1Core.Functions.GetPlayerData(function(PlayerData)
        if PlayerJob.name == "doctor" then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                N1Core.Functions.Progressbar("hospital_revive", "Reviving person..", 5000, false, true, {
                    ...
                    N1Core.Functions.Notify("You revived the person!")
    ...
    N1Core.Functions.GetPlayerData(function(PlayerData)
        if PlayerJob.name == "doctor" or PlayerJob.name == "ambulance" or PlayerJob.name == "police" then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                N1Core.Functions.TriggerCallback('hospital:GetPlayerStatus', function(result)
                    if result ~= nil then
    ...
    N1Core.Functions.GetPlayerData(function(PlayerData)
        if PlayerJob.name == "doctor" or PlayerJob.name == "ambulance" then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                N1Core.Functions.Progressbar("hospital_healwounds", "Healing wounds..", 5000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    isHealingPerson = false
                    StopAnimTask(GetPlayerPed(-1), healAnimDict, "exit", 1.0)
                    N1Core.Functions.Notify("You helped the person!")
...
function TakeOutVehicle(vehicleInfo)
    ...
    N1Core.Functions.SpawnVehicle(vehicleInfo[1], function(veh)
