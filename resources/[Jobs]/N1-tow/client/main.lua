...
AddEventHandler('jobs:client:ToggleNpc', function()
    if N1Core.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then 
            N1Core.Functions.Notify("Finish your work first!", "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
RegisterNetEvent('N1-tow:client:TowVehicle')
AddEventHandler('N1-tow:client:TowVehicle', function()
    local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), true)
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then 
            --[[ Replaced "N1Core.Functions.GetClosestVehicle()" with custom implementation "getVehicleInDirection"
                 N1Core native could not return polcice and other vehicles types (NPC) ]] 
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    N1Core.Functions.Notify("This is not the right vehicle.", "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                if vehicle ~= targetVehicle then
                    ...
                    if GetDistanceBetweenCoords(towPos.x, towPos.y, towPos.z, targetPos.x, targetPos.y, targetPos.z, true) < 11.0 then
                        N1Core.Functions.Progressbar("towing_vehicle", "Put the vehicle on it.", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                N1Core.Functions.Notify("Bring the vehicle to the hayens depot!", "success", 5000)
                            end
                            N1Core.Functions.Notify("Vehicle lifted!")
                        end, function() -- Cancel
                            StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_ped", 1.0)
            ...
            N1Core.Functions.Progressbar("untowing_vehicle", "Remove the vehicle..", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                N1Core.Functions.Notify("Removing the vehicle!")
            end, function() -- Cancel
                StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_ped", 1.0)
                N1Core.Functions.Notify("Faild!", "error")
            end)
        end
    else
        N1Core.Functions.Notify("You need to be in a tow truck first..", "error")
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do 
        Citizen.Wait(1)
        if isLoggedIn and N1Core ~= nil then
            if PlayerJob.name == "tow" then
                    ...
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, true) < 1.5) then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                                ...
                                TriggerServerEvent('N1-tow:server:DoBail', false)
                            else
                                MenuGarage()
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 4.5) then
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 1.5) then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if JobsDone > 0 then
                                RemoveBlip(CurrentBlip)
                                TriggerServerEvent("N1-tow:server:11101110", JobsDone)
                                JobsDone = 0
                                NpcOn = false
                            else
                                N1Core.Functions.Notify("You haven't done any work yet..", "error")
                            end
                        end
                ...
                if NpcOn and CurrentLocation ~= nil and next(CurrentLocation) ~= nil then
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, true) < 50.0 and not VehicleSpawned then
                        VehicleSpawned = true
                        N1Core.Functions.SpawnVehicle(CurrentLocation.model, function(veh)
...
function deliverVehicle(vehicle)
    ...
    JobsDone = JobsDone + 1
    VehicleSpawned = false
    N1Core.Functions.Notify("You have delivered a vehicle!", "success")
    N1Core.Functions.Notify("You can go pick up a new vehicle")
...
function TakeOutVehicle(vehicleInfo)
    TriggerServerEvent('N1-tow:server:DoBail', true, vehicleInfo)
    selectedVeh = vehicleInfo
end

RegisterNetEvent('N1-tow:client:SpawnVehicle')
AddEventHandler('N1-tow:client:SpawnVehicle', function()
    local vehicleInfo = selectedVeh
    local coords = Config.Locations["vehicle"].coords
    N1Core.Functions.SpawnVehicle(vehicleInfo, function(veh)
