            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        TriggerServerEvent('N1-garbagejob:server:PayShit', Earnings, location)
                        Earnings = 0
                    end
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                N1Core.Functions.TriggerCallback('N1-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        N1Core.Functions.Notify("You got back your ₹1000,- deposit!")
                                    else
                                        N1Core.Functions.Notify("You haven\'t put a deposit on this vehicle..")
                                    end
                                end)
                            end
                        else
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                N1Core.Functions.TriggerCallback('N1-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        N1Core.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            N1Core.Functions.Notify("You paid a ₹1000,- deposit!")
                                            N1Core.Functions.Notify("You started working, location has been set on your GPS!")
                                        end, coords, true)
                                    else
                                        N1Core.Functions.Notify("You don\'t have enough money for the deposit.. A deposit costs ₹1000,-")
                                    end
                                end)
                            end
                        end
                                            ...
                                            if IsControlJustPressed(0, 51) then
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        N1Core.Functions.Notify("All garbage bags are done, get to the next location!")
                                                    else
                                                        -- Hier ben je klaar met werken.
                                                        N1Core.Functions.Notify("You\'re done working! Head back to the duping ground.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        N1Core.Functions.Notify("There are still "..AmountOfBags.." bags left!")
                                                    else
                                                        N1Core.Functions.Notify("There is still "..AmountOfBags.." bag left!")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()
