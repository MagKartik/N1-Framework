...
AddEventHandler('N1Core:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    PlayerData = N1Core.Functions.GetPlayerData()
...
function StartWorking()
    N1Core.Functions.TriggerCallback('N1-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            local PlayerPed = GetPlayerPed(-1)
            ...
            UpdateUI()
            CheckLoop()
            N1Core.Functions.Notify('You have paid a 1000,- deposit!..', 'success')
        else
            N1Core.Functions.Notify('You have no money for the deposit..', 'error')
        end
    end)
end
...
RegisterNetEvent('N1-hotdogjob:client:UpdateReputation')
AddEventHandler('N1-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('N1-hotdogjob:client:ToggleSell')
AddEventHandler('N1-hotdogjob:client:ToggleSell', function(data)
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            N1Core.Functions.Notify('You ar to far from your hotdog stall..', 'error')
        end
    else
        N1Core.Functions.Notify('You have no hotdog stall', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 15.0 then
            ...
            N1Core.Functions.Notify('You're to far from the stall..', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                N1Core.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sale '..HotdogsForSale..'x voor ₹'..(HotdogsForSale * SellingPrice)..',- / [8] Afwijzen')
                if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                    N1Core.Functions.Notify(HotdogsForSale..'x Hotdog(\'s) verkocht voor ₹'..(HotdogsForSale * SellingPrice)..',-', 'success')
                    TriggerServerEvent('N1-hotdogjob:server:Sell', HotdogsForSale, SellingPrice)
                    SellingData.HasTarget = false
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    N1Core.Functions.Notify('Custommer refused!', 'error')
                    SellingData.HasTarget = false
                ...
                SellingData.Hotdog = GetAvailableHotdog()
                N1Core.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, 'You have no hotdogs.. / [8] Refuse cutommer')
            ...
            N1Core.Functions.Notify('You're to far from the stal..', 'error')
            break
        end
...
function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('N1-keyminigame:show')
    TriggerEvent('N1-keyminigame:start', FinishMinigame)
end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('N1-hotdogjob:server:UpdateReputation', Quality)
        if Config.MyLevel == 1 then
            N1Core.Functions.Notify('You made a  '..Config.Stock[Quality].Label..' Hotdog !')
            ...
            if Luck == LuckyNumber then
                N1Core.Functions.Notify('You have '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s !')
        ...
        N1Core.Functions.Notify(You have  ('..Config.Stock[Quality].Label..') no hotdogs..')
    end
    PreparingFood = false
end
...
function StopWorking()
    if DoesEntityExist(StandObject) then
        N1Core.Functions.TriggerCallback('N1-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                N1Core.Functions.Notify('You recieved your 1000,- borg!', 'success')
            else
                N1Core.Functions.Notify('Something is wrong meine freunden..', 'error')
            end
        end)
    else
        N1Core.Functions.Notify('Your stall is no where to see you wont recieve you're deposit', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
RegisterNetEvent('N1-hotdogjob:staff:DeletStand')
AddEventHandler('N1-hotdogjob:staff:DeletStand', function()
    local ped = GetPlayerPed(-1)
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            N1Core.Functions.Notify('Hotdog stal removed!')
        end
    end
end)
