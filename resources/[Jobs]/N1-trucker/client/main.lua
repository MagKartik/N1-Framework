            ...
            if PlayerJob.name == "Trucker" then
                            ...
                            if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
                                    if isTruckerVehicle(GetVehiclePedIsIn(GetPlayerPed(-1), false)) then
                                        ...
                                        TriggerServerEvent('N1-trucker:server:DoBail', false)
                                    else
                                        N1Core.Functions.Notify('This is not what u be ment to do right!', 'error')
                                    end
                                else
                                    N1Core.Functions.Notify('You need to be the driver..')
                                end
                            else
                                MenuGarage()
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 4.5) then
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 1.5) then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if JobsDone > 0 then
                                TriggerServerEvent("N1-trucker:server:01101110", JobsDone)
                                JobsDone = 0
                                ...
                                N1Core.Functions.Notify("You diddent do any work..", "error")
                            end
                        end
                            ...
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == GetVehicleNumberPlateText(vehicle) then
                                    ...
                                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, trunkpos.x, trunkpos.y, trunkpos.z, true) < 1.5 and not isWorking then
                                        ...
                                        if IsControlJustReleased(0, Keys["E"]) then
                                            isWorking = true
                                            N1Core.Functions.Progressbar("work_carrybox", "Picking box of products..", 2000, false, true, {
                                                ...
                                                isWorking = false
                                                StopAnimTask(GetPlayerPed(-1), "anim@gangops@facility@servers@", "hotwire", 1.0)
                                                N1Core.Functions.Notify("Canceld..", "error")
                                            end)
                                        end
                                    else
                            ...
                            elseif hasBox then
                                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, true) < 1.5 and not isWorking then
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        ...
                                        N1Core.Functions.Progressbar("work_dropbox", "Delivering box of products..", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                TriggerServerEvent("N1-shops:server:RestockShopItems", CurrentLocation.store)
                                                N1Core.Functions.Notify("You deliverd all products, on to the next point.")
                                                if CurrentBlip ~= nil then
                                                    RemoveBlip(CurrentBlip)
                                                    CurrentBlip = nil
                                                end
                                    ...
                                    DrawText3D(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, "Products deliverd)
                                end
                            end
                        end
                    end
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        N1Core.Functions.Notify("You went by all stores.. Time for your pay check!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
            CurrentBlip = nil
        end
...
function TakeOutVehicle(vehicleInfo)
    TriggerServerEvent('N1-trucker:server:DoBail', true, vehicleInfo)
...
RegisterNetEvent('N1-trucker:client:SpawnVehicle')
AddEventHandler('N1-trucker:client:SpawnVehicle', function()
