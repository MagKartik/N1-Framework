...
RegisterNetEvent('N1-taxi:client:DoTaxiNpc')
AddEventHandler('N1-taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
        if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                N1Core.Functions.Notify('De NPC staat aangegeven op uw navigatie!', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    N1Core.Functions.Notify('Bring the npc to the marked location.')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                N1Core.Functions.Notify('Your already doing a npc mission..')
            end
        else
            N1Core.Functions.Notify('There are non npc availble..')
        end
    else
        N1Core.Functions.Notify('Your not in a cab :(')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        ...
                        TriggerServerEvent('N1-taxi:server:NpcPay', meterData.currentFare)
                        N1Core.Functions.Notify('Deliverd the person', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterNetEvent('N1-taxi:client:toggleMeter')
AddEventHandler('N1-taxi:client:toggleMeter', function()
    local ped = GetPlayerPed(-1)
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            N1Core.Functions.Notify('This vehicle does not have a cap meter..', 'error')
        end
    else
        N1Core.Functions.Notify('Your not in a vehicle..', 'error')
    end
end)

RegisterNetEvent('N1-taxi:client:enableMeter')
AddEventHandler('N1-taxi:client:enableMeter', function()
    local ped = GetPlayerPed(-1)
    ...
    if meterIsOpen then
        ...
        N1Core.Functions.Notify('The cap meter is not active..', 'error')
    end
end)
...
RegisterNetEvent('N1-taxi:client:toggleMuis')
AddEventHandler('N1-taxi:client:toggleMuis', function()
    Citizen.Wait(400)
    if meterIsOpen then
        ...
        N1Core.Functions.Notify('No cap meter to see..', 'error')
    end
end)
...
function TakeVehicle(k)
    ...
    N1Core.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)
